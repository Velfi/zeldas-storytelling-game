...
ui_renderer_destroy :: proc(ui: ^Ui_Renderer, ctx: ^engine.Vk_Context) {
	...
	engine.vk_destroy_buffer(
		ctx,
		&ui.index_buffer,
	); engine.vk_destroy_buffer(ctx, &ui.vertex_buffer); delete(ui.vertices); delete(ui.indices); delete(ui.batches); ui^ = {}
}

ui_renderer_init :: proc(ui: ^Ui_Renderer, ctx: ^engine.Vk_Context, fallback: ^Gpu_Image) -> bool {
	ui^ =
		{}; ui.vertices = make([dynamic]Ui_Vertex, 0, 4096); ui.indices = make([dynamic]u32, 0, 6144); ui.batches = make([dynamic]Ui_Batch, 0, 64)
	...
	if !engine.vk_create_host_buffer(
		ctx,
		vk.DeviceSize(UI_MAX_INDICES * size_of(u32)),
		{.INDEX_BUFFER},
		&ui.index_buffer,
	) {ui_renderer_destroy(ui, ctx); return false}
	...
	}; if vk.CreateDescriptorSetLayout(ctx.device, &layout_info, nil, &ui.descriptor_layout) !=
	   .SUCCESS {ui_renderer_destroy(ui, ctx); return false}
	...
	}; if vk.CreateDescriptorPool(ctx.device, &pool_info, nil, &ui.descriptor_pool) !=
	   .SUCCESS {ui_renderer_destroy(ui, ctx); return false}; alloc := vk.DescriptorSetAllocateInfo {
		sType              = .DESCRIPTOR_SET_ALLOCATE_INFO,
		descriptorPool     = ui.descriptor_pool,
		descriptorSetCount = 1,
		pSetLayouts        = &ui.descriptor_layout,
	}; if vk.AllocateDescriptorSets(ctx.device, &alloc, &ui.descriptor) !=
	   .SUCCESS {ui_renderer_destroy(ui, ctx); return false}
	...
	}; if vk.CreatePipelineLayout(ctx.device, &pipeline_layout_info, nil, &ui.pipeline_layout) !=
	   .SUCCESS {ui_renderer_destroy(ui, ctx); return false}
	...
	stages := [2]vk.PipelineShaderStageCreateInfo {
	...
	}; attributes := [3]vk.VertexInputAttributeDescription {
		{
			...
			offset = u32(offset_of(Ui_Vertex, position)),
		},
		{
			location = 1,
			binding = 0,
	   ...
	   .SUCCESS {ui_renderer_destroy(ui, ctx); return false}; ui.ready = true; return true
}
...
ui_quad :: proc(
	...
	uv1 := Vec2{1, 1},
	textured := false,
	scissor := vk.Rect2D{offset = {0, 0}, extent = {0xffffffff, 0xffffffff}},
) {
...
ui_draw :: proc(ui: ^Ui_Renderer, cmd: vk.CommandBuffer, extent: vk.Extent2D) {
	if !ui.ready || len(ui.vertices) == 0 do return; mem.copy_non_overlapping(ui.vertex_buffer.mapped, raw_data(ui.vertices[:]), len(ui.vertices) * size_of(Ui_Vertex)); mem.copy_non_overlapping(ui.index_buffer.mapped, raw_data(ui.indices[:]), len(ui.indices) * size_of(u32)); viewport := vk.Viewport {
		...
		1,
		&viewport,
	); vk.CmdBindPipeline(cmd, .GRAPHICS, ui.pipeline); offset := vk.DeviceSize(0); vk.CmdBindVertexBuffers(cmd, 0, 1, &ui.vertex_buffer.handle, &offset); vk.CmdBindIndexBuffer(cmd, ui.index_buffer.handle, 0, .UINT32)
	for batch in ui.batches {scissor := batch.scissor; if scissor.extent.width == 0xffffffff do scissor = {
			offset = {0, 0},
			extent = extent,
		}; vk.CmdSetScissor(cmd, 0, 1, &scissor); descriptor := batch.descriptor; vk.CmdBindDescriptorSets(cmd, .GRAPHICS, ui.pipeline_layout, 0, 1, &descriptor, 0, nil); push := Ui_Push{{f32(extent.width), f32(extent.height)}, batch.use_texture ? 1 : 0, 0}; vk.CmdPushConstants(cmd, ui.pipeline_layout, {.VERTEX, .FRAGMENT}, 0, u32(size_of(push)), &push); vk.CmdDrawIndexed(cmd, batch.index_count, 1, batch.first_index, 0, 0)}
}
