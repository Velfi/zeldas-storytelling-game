...
story_toml_escape :: proc(value: string) -> string {
	result := strings.builder_make(allocator = context.temp_allocator)
	for ch in value {
		if ch == '\\' do _ = strings.write_string(&result, "\\\\")
...
story_toml_string_array :: proc(values: []string) -> string {
	result := strings.builder_make(allocator = context.temp_allocator)
	_ = strings.write_byte(&result, '[')
	for value, i in values {
		if i > 0 do _ = strings.write_string(&result, ", ")
		_ = fmt.sbprintf(&result, "\"%s\"", story_toml_escape(value))
	}
	_ = strings.write_byte(&result, ']')
	return strings.to_string(result)
}
story_node_choice_array :: proc(node: ^Story_Node, field: int) -> string {
	result := strings.builder_make(allocator = context.temp_allocator)
	_ = strings.write_byte(&result, '[')
	for choice, i in node.choices[:node.choice_count] {
		if i > 0 do _ = strings.write_string(&result, ", ")
		value := choice.id
		switch field {case 1:
...
story_project_serialize :: proc(project: ^Story_Project) -> string {
	...
	for capability in project.capabilities do _ = fmt.sbprintf(&builder, "\n[[capabilities]]\nid = \"%s\"\nversion = \"%s\"\n", story_toml_escape(capability.id), story_toml_escape(capability.version))
	for requirement in project.expansion_requirements {distribution := requirement.distribution == .Embed ? "embed" : "reference"; fallback := requirement.fallback == .Omit ? "omit" : "none"; _ = fmt.sbprintf(&builder, "\n[[expansions]]\nid = \"%s\"\nversion = \"%s\"\noptional = %t\ndistribution = \"%s\"\nfallback = \"%s\"\n", story_toml_escape(requirement.id), story_toml_escape(requirement.version), requirement.optional, distribution, fallback)}
	for item, i in project.entities do _ = fmt.sbprintf(&builder, "\n[[entities]]\nid = \"%s\"\nkind = \"%s\"\ndisplay_name = \"%s\"\ndescription = \"%s\"\nappearance_model_asset_ref = \"%s\"\ndefault_animation = \"%s\"\nanimation_phase = %.3f\npresentation_tint = [%d, %d, %d, %d]\npresentation_facing = %.3f\nvisibility_condition_id = \"%s\"\navailability_condition_id = \"%s\"\ncompletion_condition_id = \"%s\"\ninteraction_scene_id = \"%s\"\ninteraction_prompt = \"%s\"\nunavailable_prompt = \"%s\"\ncompletion_prompt = \"%s\"\nspace_id = \"%s\"\ntarget_kind = \"%s\"\ntarget_id = \"%s\"\nvolume = %d\ncontainer_capacity = %d\nlocked = %t\nowner = \"%s\"\ncontained_by = \"%s\"\ntags = %s\nroles = %s\n", story_toml_escape(item.id), story_toml_escape(item.kind), story_toml_escape(item.display_name), story_toml_escape(item.description), story_toml_escape(item.appearance_model_asset_ref), story_toml_escape(item.default_animation), item.animation_phase, item.presentation_tint[0], item.presentation_tint[1], item.presentation_tint[2], item.presentation_tint[3], item.presentation_facing, story_toml_escape(item.visibility_condition_id), story_toml_escape(item.availability_condition_id), story_toml_escape(item.completion_condition_id), story_toml_escape(item.interaction_scene_id), story_toml_escape(item.interaction_prompt), story_toml_escape(item.unavailable_prompt), story_toml_escape(item.completion_prompt), story_toml_escape(item.spatial.space_id), story_spatial_target_kind_text(item.spatial.target_kind), story_toml_escape(item.spatial.target_id), item.volume, item.container_capacity, item.initially_locked, story_toml_escape(item.owner_id), story_toml_escape(item.initial_container_id), story_toml_string_array(project.entities[i].tags[:item.tag_count]), story_toml_string_array(project.entities[i].roles[:item.role_count]))
	for item in project.roles do _ = fmt.sbprintf(&builder, "\n[[roles]]\nid = \"%s\"\ndisplay_name = \"%s\"\ndescription = \"%s\"\nminimum = %d\nmaximum = %d\n", story_toml_escape(item.id), story_toml_escape(item.display_name), story_toml_escape(item.description), item.minimum, item.maximum)
	for item, i in project.variables {enums := story_toml_string_array(project.variables[i].enum_values[:item.enum_value_count]); _ = fmt.sbprintf(&builder, "\n[[variables]]\nid = \"%s\"\ndisplay_name = \"%s\"\ndescription = \"%s\"\nkind = \"%s\"\nboolean_value = %t\ninteger_value = %d\ntext_value = \"%s\"\nminimum = %d\nmaximum = %d\nenum_values = %s\n", story_toml_escape(item.id), story_toml_escape(item.display_name), story_toml_escape(item.description), story_value_kind_text(item.kind), item.default_value.boolean_value, item.default_value.integer_value, story_toml_escape(item.default_value.text_value), item.minimum, item.maximum, enums)}
	for item in project.facts do _ = fmt.sbprintf(&builder, "\n[[facts]]\nid = \"%s\"\ndisplay_name = \"%s\"\nproposition = \"%s\"\nvariable = \"%s\"\ncanonical_truth = \"%s\"\nplayer_visible = %t\n", story_toml_escape(item.id), story_toml_escape(item.display_name), story_toml_escape(item.proposition), story_toml_escape(item.variable_id), story_truth_text(item.canonical_truth), item.player_visible)
	for item in project.propositions do _ = fmt.sbprintf(&builder, "\n[[propositions]]\nid = \"%s\"\ntext = \"%s\"\ncanonical_truth = \"%s\"\n", story_toml_escape(item.id), story_toml_escape(item.text), story_truth_text(item.canonical_truth))
	for item in project.initial_knowledge do _ = fmt.sbprintf(&builder, "\n[[initial_knowledge]]\nactor = \"%s\"\nproposition = \"%s\"\nstance = \"%s\"\n", story_toml_escape(item.actor_id), story_toml_escape(item.proposition_id), story_belief_text(item.stance))
	for item in project.relationships do _ = fmt.sbprintf(&builder, "\n[[relationships]]\nid = \"%s\"\nsource = \"%s\"\ntarget = \"%s\"\nkind = \"%s\"\nvariable = \"%s\"\n", story_toml_escape(item.id), story_toml_escape(item.source_id), story_toml_escape(item.target_id), story_toml_escape(item.kind), story_toml_escape(item.variable_id))
	for item, i in project.events do _ = fmt.sbprintf(&builder, "\n[[events]]\nid = \"%s\"\nsubject = \"%s\"\naction = \"%s\"\nobject = \"%s\"\nlocation = \"%s\"\nfictional_time = \"%s\"\nprovenance = \"%s\"\nwitnesses = %s\n", story_toml_escape(item.id), story_toml_escape(item.subject_id), story_toml_escape(item.action), story_toml_escape(item.object_id), story_toml_escape(item.location_id), story_toml_escape(item.fictional_time), story_toml_escape(item.provenance), story_toml_string_array(project.events[i].witnesses[:item.witness_count]))
	for item in project.objectives do _ = fmt.sbprintf(&builder, "\n[[objectives]]\nid = \"%s\"\ndisplay_name = \"%s\"\ndescription = \"%s\"\nhidden = %t\ninitial_status = \"%s\"\nstage_count = %d\ncompletion_condition_id = \"%s\"\nfailure_condition_id = \"%s\"\n", story_toml_escape(item.id), story_toml_escape(item.display_name), story_toml_escape(item.description), item.hidden, story_objective_status_text(item.initial_status), item.stage_count, story_toml_escape(item.completion_condition_id), story_toml_escape(item.failure_condition_id))
	for &item in project.conditions do _ = fmt.sbprintf(&builder, "\n[[conditions]]\nid = \"%s\"\nkind = \"%s\"\nchildren = %s\nvariable = \"%s\"\nentity = \"%s\"\nother_entity = \"%s\"\nproposition = \"%s\"\nobjective = \"%s\"\nevent = \"%s\"\ncontent = \"%s\"\ntext = \"%s\"\nspace_a = \"%s\"\ntarget_a = \"%s\"\nspace_b = \"%s\"\ntarget_b = \"%s\"\ndistance = %.3f\nvalue_kind = \"%s\"\nboolean_value = %t\ninteger_value = %d\ntext_value = \"%s\"\ncomparison = %d\nobjective_status = %d\nstance = \"%s\"\n", story_toml_escape(item.id), story_condition_kind_text(item.kind), story_toml_string_array(item.child_ids[:item.child_id_count]), story_toml_escape(item.variable_id), story_toml_escape(item.entity_id), story_toml_escape(item.other_entity_id), story_toml_escape(item.proposition_id), story_toml_escape(item.objective_id), story_toml_escape(item.event_id), story_toml_escape(item.content_id), story_toml_escape(item.text_value), story_toml_escape(item.spatial_a.space_id), story_toml_escape(item.spatial_a.target_id), story_toml_escape(item.spatial_b.space_id), story_toml_escape(item.spatial_b.target_id), item.distance, story_value_kind_text(item.value.kind), item.value.boolean_value, item.value.integer_value, story_toml_escape(item.value.text_value), int(item.comparison), int(item.objective_status), story_belief_text(item.belief_stance))
	for item in project.effects do _ = fmt.sbprintf(&builder, "\n[[effects]]\nid = \"%s\"\nkind = \"%s\"\nvariable = \"%s\"\nactor = \"%s\"\nother_actor = \"%s\"\nproposition = \"%s\"\nobjective = \"%s\"\nevent = \"%s\"\ncontent = \"%s\"\ncommand = \"%s\"\nspace = \"%s\"\ntarget = \"%s\"\ndestination_space = \"%s\"\ndestination = \"%s\"\nvalue_kind = \"%s\"\nboolean_value = %t\ninteger_value = %d\ntext_value = \"%s\"\nstance = \"%s\"\nobjective_status = %d\nworld_enabled = %t\n", story_toml_escape(item.id), story_effect_kind_text(item.kind), story_toml_escape(item.variable_id), story_toml_escape(item.actor_id), story_toml_escape(item.other_actor_id), story_toml_escape(item.proposition_id), story_toml_escape(item.objective_id), story_toml_escape(item.event_id), story_toml_escape(item.content_id), story_spatial_command_text(item.spatial_command), story_toml_escape(item.spatial_target.space_id), story_toml_escape(item.spatial_target.target_id), story_toml_escape(item.spatial_destination.space_id), story_toml_escape(item.spatial_destination.target_id), story_value_kind_text(item.value.kind), item.value.boolean_value, item.value.integer_value, story_toml_escape(item.value.text_value), story_belief_text(item.belief_stance), int(item.objective_status), item.world_enabled)
	for item in project.localizations do _ = fmt.sbprintf(&builder, "\n[[localizations]]\nnode = \"%s\"\nlanguage = \"%s\"\ntext = \"%s\"\nstatus = \"%s\"\nnote = \"%s\"\nvoice = \"%s\"\n", story_toml_escape(item.node_id), story_toml_escape(item.language), story_toml_escape(item.text), story_toml_escape(item.status), story_toml_escape(item.note), story_toml_escape(item.voice))
	for item in project.scenes do _ = fmt.sbprintf(&builder, "\n[[scenes]]\nid = \"%s\"\ndisplay_name = \"%s\"\nentry_node = \"%s\"\nbound_entity = \"%s\"\nsummary = \"%s\"\nreturn_to = \"%s\"\n", story_toml_escape(item.id), story_toml_escape(item.display_name), story_toml_escape(item.entry_node), story_toml_escape(item.bound_entity), story_toml_escape(item.summary), story_toml_escape(item.return_to))
	for &item in project.nodes do _ = fmt.sbprintf(&builder, "\n[[nodes]]\nid = \"%s\"\nscene = \"%s\"\nkind = \"%s\"\nline_id = \"%s\"\nspeaker = \"%s\"\ntext = \"%s\"\nnext = \"%s\"\nsuccess = \"%s\"\nfailure = \"%s\"\ncancel = \"%s\"\nsubscene = \"%s\"\ncondition_id = \"%s\"\neffect_ids = %s\nui = \"%s\"\ncamera = \"%s\"\nactor = \"%s\"\nactor_mark = \"%s\"\nanimation = \"%s\"\nui_image_asset_ref = \"%s\"\nsound_cue_asset_ref = \"%s\"\nanimation_asset_ref = \"%s\"\nsummary = \"%s\"\nending = \"%s\"\ndomain_ref = \"%s\"\nevent_id = \"%s\"\nduration = %.3f\ntransition = %.3f\nblocking = %t\nchoice_ids = %s\nchoice_labels = %s\nchoice_targets = %s\nchoice_conditions = %s\n", story_toml_escape(item.id), story_toml_escape(item.scene_id), story_node_kind_text(item.kind), story_toml_escape(item.line_id), story_toml_escape(item.speaker_id), story_toml_escape(item.text), story_toml_escape(item.next), story_toml_escape(item.success), story_toml_escape(item.failure), story_toml_escape(item.cancel), story_toml_escape(item.subscene_id), story_toml_escape(item.condition_id), story_toml_string_array(item.effect_ids[:item.effect_id_count]), story_toml_escape(item.ui), story_toml_escape(item.camera), story_toml_escape(item.actor), story_toml_escape(item.actor_mark), story_toml_escape(item.animation), story_toml_escape(item.ui_image_asset_ref), story_toml_escape(item.sound_cue_asset_ref), story_toml_escape(item.animation_asset_ref), story_toml_escape(item.summary), story_toml_escape(item.ending), story_toml_escape(item.domain_ref), story_toml_escape(item.event_id), item.duration, item.transition, item.blocking, story_node_choice_array(&item, 0), story_node_choice_array(&item, 1), story_node_choice_array(&item, 2), story_node_choice_array(&item, 3))
	for item in project.storylet_groups do _ = fmt.sbprintf(&builder, "\n[[storylet_groups]]\nid = \"%s\"\nallow_empty = %t\nseeded_random_ties = %t\n", story_toml_escape(item.id), item.allow_empty, item.seeded_random_ties)
	for &item in project.storylets do _ = fmt.sbprintf(&builder, "\n[[storylets]]\nid = \"%s\"\ngroup = \"%s\"\nscene = \"%s\"\ncondition_ids = %s\neffect_ids = %s\npriority = %d\nspecificity = %d\ncooldown = %d\nauthored_order = %d\nrepeat = \"%s\"\nfallback = %t\n", story_toml_escape(item.id), story_toml_escape(item.group), story_toml_escape(item.scene_id), story_toml_string_array(item.condition_ids[:item.condition_count]), story_toml_string_array(item.effect_ids[:item.effect_count]), item.dramatic_priority, item.specificity, item.cooldown, item.authored_order, story_repeat_text(item.repeat_policy), item.fallback)
	for item in project.endings do _ = fmt.sbprintf(&builder, "\n[[endings]]\nid = \"%s\"\ntitle = \"%s\"\nsummary = \"%s\"\ncondition_id = \"%s\"\npriority = %d\n", story_toml_escape(item.id), story_toml_escape(item.title), story_toml_escape(item.summary), story_toml_escape(item.condition_id), item.priority)
	for item in project.invariants do _ = fmt.sbprintf(&builder, "\n[[invariants]]\nid = \"%s\"\ndescription = \"%s\"\nkind = \"%s\"\ncondition_id = \"%s\"\nrequired = %t\n", story_toml_escape(item.id), story_toml_escape(item.description), story_invariant_kind_text(item.kind), story_toml_escape(item.condition_id), item.required)
	for capability in project.capabilities {adapter := story_domain_find(capability.id, capability.version); if capability.payload != nil && adapter != nil && adapter.serialize != nil {capability_text := adapter.serialize(capability.payload); if capability_text != "" do _ = fmt.sbprintf(&builder, "\n%s", capability_text)}}
