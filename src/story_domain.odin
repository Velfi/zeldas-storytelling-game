...
story_domain_adapter_complete :: proc(adapter:Story_Domain_Adapter)->bool {
	...
	return adapter.parse!=nil&&adapter.clone!=nil&&adapter.destroy!=nil&&adapter.validate!=nil&&adapter.compile!=nil&&adapter.hash!=nil&&adapter.serialize!=nil&&adapter.state_create!=nil&&adapter.state_clone!=nil&&adapter.state_destroy!=nil
}
...
story_domain_register :: proc(adapter:Story_Domain_Adapter)->bool {
	...
	story_domain_adapters[story_domain_adapter_count]=adapter;story_domain_adapter_count+=1;return true
}
...
story_capability_payload :: proc(project:^Story_Project,id,version:string)->rawptr {
	i:=story_capability_index(project,id,version);if i<0 do return nil;return project.capabilities[i].payload
}
