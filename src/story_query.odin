...
Story_Capabilities :: struct {
	...
	domain_count:                                                                             int,
	revision_checked_commands,
	atomic_batches,
	dry_run,
	deterministic_compile,
	scene_runtime: bool,
}
Story_Domain_Capability :: struct {
	id, version: string,
}

Story_Reference :: struct {
	id, display_name: string,
	kind:             Story_Record_Kind,
}
Story_Reference_Query :: struct {
	items: [dynamic]Story_Reference,
}
Story_Dependency_Query :: struct {
	target_id:  string,
	dependents: [dynamic]Story_Reference,
}

story_capabilities :: proc() -> Story_Capabilities {story_domain_register_core(); result :=
		Story_Capabilities {
			format_version            = STORY_PROJECT_VERSION,
			revision_checked_commands = true,
			atomic_batches            = true,
			dry_run                   = true,
			deterministic_compile     = true,
			scene_runtime             = true,
		}
	for 	adapter, i in story_domain_adapters[:story_domain_adapter_count] {result.domains[i] = {
			adapter.id,
			adapter.version,
		}
...
story_reference_matches :: proc(id, name, needle: string) -> bool {return(
		needle == "" ||
		strings.contains(strings.to_lower(id), strings.to_lower(needle)) ||
		strings.contains(strings.to_lower(name), strings.to_lower(needle)) \
	)}
...
story_reference_query_destroy :: proc(result: ^Story_Reference_Query) {delete(result.items)
	result^ = {}}

story_dependency_add :: proc(
	result: ^Story_Dependency_Query,
	id, name: string,
	kind: Story_Record_Kind,
) {for item in result.dependents do if item.id == id && item.kind == kind do return; append(
		&result.dependents,
		Story_Reference{id, name, kind},
	)}
...
story_dependency_query_destroy :: proc(result: ^Story_Dependency_Query) {delete(result.dependents)
	result^ = {}}
