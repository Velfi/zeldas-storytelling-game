...
story_value_boolean :: proc(value: bool) -> Story_Value {return{
		kind = .Boolean,
		boolean_value = value,
	}}
story_value_integer :: proc(value: int) -> Story_Value {return{
		kind = .Integer,
		integer_value = value,
	}}
story_value_text :: proc(kind: Story_Value_Kind, value: string) -> Story_Value {return{
		kind = kind,
		text_value = value,
	}}
...
story_value_equal :: proc(a, b: Story_Value) -> bool {
	...
	switch a.kind {case .Boolean:
		return a.boolean_value == b.boolean_value; case .Integer:
		return a.integer_value == b.integer_value; case .Enumeration, .Entity:
		return a.text_value == b.text_value}
	return false
}

story_value_string :: proc(value: Story_Value) -> string {
	switch value.kind {case .Boolean:
		return value.boolean_value ? "true" : "false"; case .Integer:
		return fmt.tprintf("%d", value.integer_value); case .Enumeration, .Entity:
		return value.text_value}
	return ""
}

story_entity_index :: proc(project: ^Story_Project, id: string) -> int {for item, i in project.entities do if item.id == id do return i
	return -1}
story_variable_index :: proc(project: ^Story_Project, id: string) -> int {for item, i in project.variables do if item.id == id do return i
	return -1}
story_proposition_index :: proc(project: ^Story_Project, id: string) -> int {for item, i in project.propositions do if item.id == id do return i
	return -1}
story_objective_index :: proc(project: ^Story_Project, id: string) -> int {for item, i in project.objectives do if item.id == id do return i
	return -1}
story_event_index :: proc(project: ^Story_Project, id: string) -> int {for item, i in project.events do if item.id == id do return i
	return -1}
story_condition_index :: proc(project: ^Story_Project, id: string) -> int {for item, i in project.conditions do if item.id == id do return i
	return -1}
story_effect_index :: proc(project: ^Story_Project, id: string) -> int {for item, i in project.effects do if item.id == id do return i
	return -1}
story_scene_index :: proc(project: ^Story_Project, id: string) -> int {for item, i in project.scenes do if item.id == id do return i
	return -1}

story_state_value_index :: proc(state: ^Story_State, id: string) -> int {for item, i in state.values do if item.variable_id == id do return i
	return -1}
story_state_knowledge_index :: proc(state: ^Story_State, actor, proposition: string) -> int {for item, i in state.knowledge do if item.actor_id == actor && item.proposition_id == proposition do return i
	return -1}
story_state_objective_index :: proc(state: ^Story_State, id: string) -> int {for item, i in state.objectives do if item.objective_id == id do return i
	return -1}
story_tracked_objective_index :: proc(project: ^Story_Project, state: ^Story_State) -> int {
	...
	for objective, project_index in project.objectives {
		...
		state_index := story_state_objective_index(
			state,
			objective.id,
		); if state_index < 0 || state.objectives[state_index].status != .Active do continue
...
story_state_history_index :: proc(items: []Story_Content_History, id: string) -> int {for item, i in items do if item.id == id do return i
	return -1}
...
story_value_valid :: proc(variable: ^Story_Variable, value: Story_Value) -> bool {
	...
	case .Boolean:
		return true
	case .Integer:
		return value.integer_value >= variable.minimum && value.integer_value <= variable.maximum
	case .Enumeration:
...
story_state_new :: proc(project: ^Story_Project) -> Story_State {
	state := Story_State {
		...
		schema_identity = story_schema_identity(project),
		seed            = 1,
	}
...
story_state_clone :: proc(source: ^Story_State) -> Story_State {
	result := source^
	result.values =
		nil; result.knowledge = nil; result.communications = nil; result.objectives = nil; result.emitted_events = nil; result.completed_scenes = nil; result.storylet_history = nil; result.world = nil; result.containers = nil; result.containment = nil
	append(
		&result.values,
		..source.values[:],
	); append(&result.knowledge, ..source.knowledge[:]); append(&result.communications, ..source.communications[:]); append(&result.objectives, ..source.objectives[:]); append(&result.emitted_events, ..source.emitted_events[:]); append(&result.completed_scenes, ..source.completed_scenes[:]); append(&result.storylet_history, ..source.storylet_history[:]); append(&result.world, ..source.world[:]); append(&result.containers, ..source.containers[:]); append(&result.containment, ..source.containment[:])
	return result
}

story_state_destroy :: proc(state: ^Story_State) {delete(state.values); delete(state.knowledge)
	delete(state.communications)
	delete(state.objectives)
	delete(state.emitted_events)
	delete(state.completed_scenes)
	delete(state.storylet_history)
	delete(state.world)
	delete(state.containers)
	delete(state.containment)
	state^ = {}}

story_entity_has_tag :: proc(project: ^Story_Project, id, tag: string) -> bool {i :=
		story_entity_index(project, id)
	if i < 0 do return false
	for item in project.entities[i].tags[:project.entities[i].tag_count] do if item == tag do return true
	return false}
story_entity_has_role :: proc(project: ^Story_Project, id, role: string) -> bool {i :=
		story_entity_index(project, id)
	if i < 0 do return false
	for item in project.entities[i].roles[:project.entities[i].role_count] do if item == role do return true
	return false}

story_condition_eval :: proc(
	...
	case .Not:
		...
		r := story_condition_eval(
			project,
			state,
			story_condition_index(project, c.child_ids[0]),
			depth + 1,
			spatial,
		); return {!r.value, fmt.tprintf("not (%s)", r.explanation)}
	case .Value_Equals:
		i := story_state_value_index(state, c.variable_id)
		if i < 0 do return {false, fmt.tprintf("%s is undefined", c.variable_id)}
		ok := story_value_equal(state.values[i].value, c.value)
		return {
			ok,
			fmt.tprintf("%s is %s", c.variable_id, story_value_string(state.values[i].value)),
		}
	case .Integer_Compare:
		i := story_state_value_index(state, c.variable_id)
		if i < 0 || state.values[i].value.kind != .Integer do return {false, fmt.tprintf("%s is not an integer", c.variable_id)}
		a, b := state.values[i].value.integer_value, c.value.integer_value
		ok := false
		switch c.comparison {case .Equal:
			ok = a == b; case .Not_Equal:
			ok = a != b; case .Less:
			ok = a < b; case .Less_Equal:
			ok = a <= b; case .Greater:
			ok = a > b; case .Greater_Equal:
			ok = a >= b}
		return {ok, fmt.tprintf("%s is %d", c.variable_id, a)}
	case .Entity_Has_Tag:
		return {
			story_entity_has_tag(project, c.entity_id, c.text_value),
			fmt.tprintf("%s has tag %s", c.entity_id, c.text_value),
		}
	case .Entity_Has_Role:
		return {
			story_entity_has_role(project, c.entity_id, c.text_value),
			fmt.tprintf("%s has role %s", c.entity_id, c.text_value),
		}
	case .Aware, .Unaware, .Belief_Equals:
		...
		ok :=
			aware &&
			stance ==
				c.belief_stance; return {ok, fmt.tprintf("%s stance for %s is %v", c.entity_id, c.proposition_id, stance)}
	case .Communicated:
	...
	case .Objective_Equals:
		i := story_state_objective_index(state, c.objective_id)
		ok := i >= 0 && state.objectives[i].status == c.objective_status
		return {ok, fmt.tprintf("objective %s status checked", c.objective_id)}
	case .Event_Occurred:
	...
	case .Scene_Completed:
		return {
			story_state_history_index(state.completed_scenes[:], c.content_id) >= 0,
			fmt.tprintf("scene %s completion checked", c.content_id),
		}
	case .Storylet_Seen:
		return {
			story_state_history_index(state.storylet_history[:], c.content_id) >= 0,
			fmt.tprintf("storylet %s history checked", c.content_id),
		}
	case .Capability_State:
		return {false, "capability conditions require StoryRuntime evaluation"}
...
story_trace_add :: proc(
	trace: ^Story_Trace,
	subject, before, after, explanation: string,
) {if trace.count >= len(trace.items) do return; trace.items[trace.count] = {
		subject,
		before,
		after,
		explanation,
...
story_history_mark :: proc(
	items: ^[dynamic]Story_Content_History,
	id: string,
	sequence: u64,
) {i := story_state_history_index(items^[:], id); if i < 0 {append(
			items,
			Story_Content_History{id = id, count = 1, last_sequence = sequence},
		)
		return}
	items[i].count += 1
	items[i].last_sequence = sequence}

story_effect_apply :: proc(
	project: ^Story_Project,
	state: ^Story_State,
	effect: Story_Effect,
	trace: ^Story_Trace,
	...
	case .Set_Value, .Add_Integer:
		vi := story_variable_index(project, effect.variable_id)
		si := story_state_value_index(state, effect.variable_id)
		if vi < 0 || si < 0 do return false
		before := story_value_string(state.values[si].value)
		next := effect.value
		if effect.kind == .Add_Integer {if project.variables[vi].kind != .Integer do return false
			next = story_value_integer(
				state.values[si].value.integer_value + effect.value.integer_value,
			)}
		if !story_value_valid(&project.variables[vi], next) do return false
		state.values[si].value = next
		story_trace_add(
			trace,
			effect.variable_id,
			before,
			story_value_string(next),
			"typed value effect",
		)
		return true
	case .Make_Aware, .Set_Belief:
		...
		i := story_state_knowledge_index(state, effect.actor_id, effect.proposition_id)
		before := "unaware"
		if i <
		   0 {append(&state.knowledge, Story_Knowledge{effect.actor_id, effect.proposition_id, .Uncertain})
			i =
				len(state.knowledge) -
				1} else do before = fmt.tprintf("%v", state.knowledge[i].stance)
		if effect.kind == .Set_Belief do state.knowledge[i].stance = effect.belief_stance
		story_trace_add(
			trace,
			fmt.tprintf("%s/%s", effect.actor_id, effect.proposition_id),
			before,
			fmt.tprintf("%v", state.knowledge[i].stance),
			"epistemic effect",
		)
		return true
	case .Communicate:
		...
		append(
			...
			Story_Communication {
				...
				sender_id = effect.actor_id,
				recipient_id = effect.other_actor_id,
				proposition_id = effect.proposition_id,
				scene_id = effect.content_id,
			},
		)
		i := story_state_knowledge_index(state, effect.other_actor_id, effect.proposition_id)
		if i <
		   0 {append(&state.knowledge, Story_Knowledge{effect.other_actor_id, effect.proposition_id, .Uncertain})
			i = len(state.knowledge) - 1}
		state.knowledge[i].stance = effect.belief_stance
		story_trace_add(
			trace,
			effect.proposition_id,
			"not communicated",
			"communicated",
			fmt.tprintf("%s told %s", effect.actor_id, effect.other_actor_id),
		)
		return true
	case .Set_Objective:
		i := story_state_objective_index(state, effect.objective_id); if i < 0 do return false
		before_status := state.objectives[i].status
		before := fmt.tprintf("%v", before_status)
		if before_status != effect.objective_status {
			state.sequence += 1; state.objectives[i].status = effect.objective_status
			if effect.objective_status ==
			   .Active {state.objectives[i].activated_sequence = state.sequence
				state.objectives[i].completed_sequence = 0}
		...
		story_trace_add(
			...
			effect.objective_id,
			before,
			fmt.tprintf("%v", effect.objective_status),
			"objective effect",
		)
		return true
	case .Emit_Event:
		if story_event_index(project, effect.event_id) < 0 do return false
		append(&state.emitted_events, effect.event_id)
		state.sequence += 1
		story_trace_add(trace, effect.event_id, "pending", "occurred", "event emitted")
		return true
	case .Complete_Scene:
		state.sequence += 1
		story_history_mark(&state.completed_scenes, effect.content_id, state.sequence)
		story_trace_add(trace, effect.content_id, "incomplete", "completed", "scene completion")
		return true
	case .Mark_Storylet:
		state.sequence += 1
		story_history_mark(&state.storylet_history, effect.content_id, state.sequence)
		story_trace_add(trace, effect.content_id, "unseen", "seen", "storylet history")
		return true
	case .Spatial_Command:
		if !story_spatial_stage_effect(spatial, effect) do return false
		story_trace_add(
			trace,
			effect.spatial_target.target_id,
			"world state",
			"staged",
			"typed spatial command",
		); return true
	}
	return false
}
...
story_apply_transaction :: proc(
	...
	if spatial != nil &&
	   spatial.begin !=
		   nil {spatial_started = spatial.begin(spatial.userdata); if !spatial_started {story_state_destroy(&working); result.message = "spatial transaction could not begin"; return result}}
	for index in effect_indices {
		if index < 0 ||
		   index >= len(project.effects) ||
		   !story_effect_apply(
				   project,
				   &working,
				   ...
				   &result.trace,
				   spatial,
			   ) {if spatial_started && spatial.rollback != nil do spatial.rollback(spatial.userdata); story_state_destroy(&working); result.message = fmt.tprintf("effect %d failed; transaction rolled back", index); return result}
	}
	if spatial_started &&
	   spatial.commit != nil &&
	   !spatial.commit(
			   spatial.userdata,
		   ) {if spatial.rollback != nil do spatial.rollback(spatial.userdata); story_state_destroy(&working); result.message = "spatial commit failed; transaction rolled back"; return result}
	story_state_destroy(
		state,
	); state^ = working; result.ok = true; result.message = fmt.tprintf("applied %d effects", len(effect_indices)); return result
}

Storylet_Candidate :: struct {
	index, priority, specificity, seen_count, authored_order: int,
	eligible, selected:                                       bool,
	explanation:                                              string,
}
Storylet_Selection :: struct {
	found:          bool,
	storylet_index: int,
	candidates:     [dynamic]Storylet_Candidate,
	explanation:    string,
}

storylet_select :: proc(
	project: ^Story_Project,
	state: ^Story_State,
	group: string,
) -> Storylet_Selection {
	result := Storylet_Selection {
		storylet_index = -1,
	}; best: Storylet_Candidate; has_best := false
	for item, index in project.storylets {
		if item.group != group do continue
		candidate := Storylet_Candidate {
			index          = index,
			priority       = item.dramatic_priority,
			specificity    = item.specificity,
			authored_order = item.authored_order,
			eligible       = true,
		}
		history := story_state_history_index(
			state.storylet_history[:],
			item.id,
		); if history >= 0 {candidate.seen_count = state.storylet_history[history].count; if item.repeat_policy == .Once {candidate.eligible = false; candidate.explanation = "once-only storylet was already seen"}; if item.repeat_policy == .Cooldown && int(state.sequence - state.storylet_history[history].last_sequence) < item.cooldown {candidate.eligible = false; candidate.explanation = "storylet cooldown is active"}}
		...
		better :=
			...
			candidate.priority == best.priority &&
				...
				candidate.seen_count < best.seen_count ||
			candidate.priority == best.priority &&
				candidate.specificity == best.specificity &&
				candidate.seen_count == best.seen_count &&
...
storylet_selection_destroy :: proc(result: ^Storylet_Selection) {delete(result.candidates)
	result^ = {}}

story_validation_add :: proc(
	result: ^Story_Validation,
	severity: Story_Diagnostic_Severity,
	id, message: string,
) {append(&result.diagnostics, Story_Diagnostic{severity, id, message}); if severity == .Error do result.error_count += 1
	else if severity == .Warning do result.warning_count += 1}
story_validation_claim_id :: proc(
	result: ^Story_Validation,
	ids: ^[dynamic]string,
	id, kind: string,
) {
	if id ==
	   "" {story_validation_add(result, .Error, "", fmt.tprintf("%s requires an ID", kind)); return}
...
story_project_assign_legacy_ids :: proc(project: ^Story_Project) {
	...
	for &node in project.nodes {
		...
		if node.effect_id_count == 0 &&
		   node.effect_count > 0 &&
		   node.first_effect >= 0 &&
		   node.first_effect + node.effect_count <=
			   len(
				   project.effects,
			   ) {node.effect_id_count = min(node.effect_count, len(node.effect_ids)); for i in 0 ..< node.effect_id_count do node.effect_ids[i] = project.effects[node.first_effect + i].id}
	}
...
story_condition_validate_node :: proc(
	project: ^Story_Project,
	result: ^Story_Validation,
	index, depth: int,
) {
	if index < 0 ||
	   index >=
		   len(
			   project.conditions,
		   ) {story_validation_add(result, .Error, "condition", "condition reference is out of range"); return}
	if depth >
	   64 {story_validation_add(result, .Error, "condition", "condition nesting exceeds 64 levels"); return}
	c := project.conditions[index]
	if c.kind == .All || c.kind == .Any || c.kind == .Not {
		if c.child_id_count <=
		   0 {story_validation_add(result, .Error, c.id, "condition group has no children"); return}
	...
	if c.kind == .Capability_State {
		adapter: ^Story_Domain_Adapter
		for capability in project.capabilities do if capability.id == c.entity_id {adapter = story_domain_find(capability.id, capability.version); break}
		if adapter == nil do story_validation_add(result, .Error, c.id, "capability condition references an undeclared or unsupported capability")
		else if adapter.condition_eval == nil do story_validation_add(result, .Error, c.id, "capability does not support authored conditions")
		if strings.trim_space(c.content_id) == "" do story_validation_add(result, .Error, c.id, "capability condition requires a query")
	}
...
story_node_index :: proc(project: ^Story_Project, scene, id: string) -> int {for node, i in project.nodes do if node.scene_id == scene && node.id == id do return i
	return -1}

story_scene_reaches :: proc(
	project: ^Story_Project,
...
story_project_validate :: proc(project: ^Story_Project) -> Story_Validation {
	...
	for entity in project.entities {
		...
		if story_entity_has_role(project, entity.id, "world_entity") {
			if !story_spatial_id_valid({entity.spatial.space_id, entity.spatial.target_id}) do story_validation_add(&result, .Error, entity.id, "world_entity role requires a complete spatial binding")
			if strings.trim_space(entity.display_name) == "" || strings.trim_space(entity.kind) == "" do story_validation_add(&result, .Error, entity.id, "world_entity role requires complete presentation metadata")
		}
		if story_entity_has_role(project, entity.id, "interactive") {
			if !story_spatial_id_valid({entity.spatial.space_id, entity.spatial.target_id}) do story_validation_add(&result, .Error, entity.id, "interactive role requires a complete spatial binding")
			if entity.interaction_scene_id == "" || strings.trim_space(entity.interaction_prompt) == "" do story_validation_add(&result, .Error, entity.id, "interactive role requires scene and prompt bindings")
		}
		condition_refs := [3]string {
			entity.visibility_condition_id,
			entity.availability_condition_id,
			entity.completion_condition_id,
		}; for condition_id in condition_refs do if condition_id != "" && story_condition_index(project, condition_id) < 0 do story_validation_add(&result, .Error, entity.id, "entity presentation references an unknown condition")
		if entity.interaction_scene_id != "" && story_scene_index(project, entity.interaction_scene_id) < 0 do story_validation_add(&result, .Error, entity.id, "entity interaction references an unknown scene")
	...
	for node in project.nodes {
		...
		if node.line_id !=
		   "" {for line in line_ids do if line == node.line_id do story_validation_add(&result, .Error, node.id, "duplicate stable line ID"); append(&line_ids, node.line_id)}
		targets := [4]string {
			node.next,
			node.success,
			node.failure,
			node.cancel,
		}; for target in targets do if target != "" && story_node_index(project, node.scene_id, target) < 0 do story_validation_add(&result, .Error, node.id, "node edge references an unknown target")
		if node.kind ==
		   .Subscene {if story_scene_index(project, node.subscene_id) < 0 do story_validation_add(&result, .Error, node.id, "subscene node references unknown scene"); visiting: [128]string; count := 0; if story_scene_reaches(project, node.subscene_id, node.scene_id, &visiting, &count) do story_validation_add(&result, .Error, node.id, "recursive subscene call is not allowed")}
	...
	for item, i in project.localizations {
		found :=
			false; for node in project.nodes do if node.id == item.node_id do found = true; if !found do story_validation_add(&result, .Error, item.node_id, "localization references an unknown node")
...
Story_Command_Batch_Result :: struct {
	ok, stale:               bool,
	revision:                u64,
	message, impact_summary: string,
	changed_ids:             [STORY_MAX_CHANGED_IDS]string,
	changed_count:           int,
...
story_project_clone :: proc(source: ^Story_Project) -> Story_Project {
	result := source^
	result.expansion_requirements =
		nil; append(&result.expansion_requirements, ..source.expansion_requirements[:])
	result.capabilities =
		nil; for capability in source.capabilities {copy := capability; copy.payload = nil; adapter := story_domain_find(capability.id, capability.version); if capability.payload != nil && adapter != nil && adapter.clone != nil do copy.payload = adapter.clone(capability.payload); append(&result.capabilities, copy)}
	result.resolved_environment.expansions =
		nil; append(&result.resolved_environment.expansions, ..source.resolved_environment.expansions[:])
	result.entities =
		nil; result.roles = nil; result.variables = nil; result.facts = nil; result.propositions = nil; result.initial_knowledge = nil; result.relationships = nil; result.events = nil; result.objectives = nil; result.conditions = nil; result.effects = nil; result.localizations = nil; result.scenes = nil; result.nodes = nil; result.storylet_groups = nil; result.storylets = nil; result.endings = nil; result.invariants = nil
	append(
		&result.entities,
		..source.entities[:],
	); append(&result.roles, ..source.roles[:]); append(&result.variables, ..source.variables[:]); append(&result.facts, ..source.facts[:]); append(&result.propositions, ..source.propositions[:]); append(&result.initial_knowledge, ..source.initial_knowledge[:]); append(&result.relationships, ..source.relationships[:]); append(&result.events, ..source.events[:]); append(&result.objectives, ..source.objectives[:]); append(&result.conditions, ..source.conditions[:]); append(&result.effects, ..source.effects[:]); append(&result.localizations, ..source.localizations[:]); append(&result.scenes, ..source.scenes[:]); append(&result.nodes, ..source.nodes[:]); append(&result.storylet_groups, ..source.storylet_groups[:]); append(&result.storylets, ..source.storylets[:]); append(&result.endings, ..source.endings[:]); append(&result.invariants, ..source.invariants[:]); return result
}

story_project_destroy :: proc(project: ^Story_Project) {for 	capability in project.capabilities {adapter := story_domain_find(capability.id, capability.version)
		if capability.payload != nil && adapter != nil && adapter.destroy != nil do adapter.destroy(capability.payload)}
	delete(project.expansion_requirements)
	delete(project.capabilities)
	delete(project.resolved_environment.expansions)
	delete(project.entities)
	delete(project.roles)
	delete(project.variables)
	delete(project.facts)
	delete(project.propositions)
	delete(project.initial_knowledge)
	delete(project.relationships)
	delete(project.events)
	delete(project.objectives)
	delete(project.conditions)
	delete(project.effects)
	delete(project.localizations)
	delete(project.scenes)
	delete(project.nodes)
	delete(project.storylet_groups)
	delete(project.storylets)
	delete(project.endings)
	delete(project.invariants)
	project^ = {}}

story_command_record_id :: proc(command: ^Story_Command) -> string {switch
	command.record_kind {case .Entity:
		...
		return command.objective.id; case .Scene:
		return command.scene.id; case .Node:
		return command.node.id; case .Storylet_Group:
		return command.storylet_group.id; case .Storylet:
		return command.storylet.id; case .Ending:
		return command.ending.id; case .Invariant:
		return command.invariant.id}
	return ""}
story_command_id :: proc(command: ^Story_Command) -> string {switch command.kind {case .Add_Entity:
		return command.entity.id; case .Add_Variable:
		return command.variable.id; case .Add_Proposition:
		return command.proposition.id; case .Add_Scene:
		return command.scene.id; case .Add_Storylet:
		return command.storylet.id; case .Add_Record, .Update_Record:
		return story_command_record_id(command); case .Delete_By_Id:
		return command.id}; return ""}

story_apply_record :: proc(
	project: ^Story_Project,
	...
	case .Entity:
		for &item in project.entities do if item.id == command.entity.id {if !update do return false; item = command.entity; return true}
		if update do return false
		append(&project.entities, command.entity)
	case .Role:
		for &item in project.roles do if item.id == command.role.id {if !update do return false; item = command.role; return true}
		if update do return false
		append(&project.roles, command.role)
	case .Variable:
		for &item in project.variables do if item.id == command.variable.id {if !update do return false; item = command.variable; return true}
		if update do return false
		append(&project.variables, command.variable)
	case .Fact:
		for &item in project.facts do if item.id == command.fact.id {if !update do return false; item = command.fact; return true}
		if update do return false
		append(&project.facts, command.fact)
	case .Proposition:
		for &item in project.propositions do if item.id == command.proposition.id {if !update do return false; item = command.proposition; return true}
		if update do return false
		append(&project.propositions, command.proposition)
	case .Knowledge:
		for &item in project.initial_knowledge do if item.actor_id == command.knowledge.actor_id && item.proposition_id == command.knowledge.proposition_id {if !update do return false; item = command.knowledge; return true}
		if update do return false
		append(&project.initial_knowledge, command.knowledge)
	case .Relationship:
		for &item in project.relationships do if item.id == command.relationship.id {if !update do return false; item = command.relationship; return true}
		if update do return false
		append(&project.relationships, command.relationship)
	case .Event:
		for &item in project.events do if item.id == command.event.id {if !update do return false; item = command.event; return true}
		if update do return false
		append(&project.events, command.event)
	case .Objective:
		for &item in project.objectives do if item.id == command.objective.id {if !update do return false; item = command.objective; return true}
		if update do return false
		append(&project.objectives, command.objective)
	case .Scene:
		for &item in project.scenes do if item.id == command.scene.id {if !update do return false; item = command.scene; return true}
		if update do return false
		append(&project.scenes, command.scene)
	case .Node:
		for &item in project.nodes do if item.id == command.node.id {if !update do return false; item = command.node; return true}
		if update do return false
		append(&project.nodes, command.node)
	case .Storylet_Group:
		for &item in project.storylet_groups do if item.id == command.storylet_group.id {if !update do return false; item = command.storylet_group; return true}
		if update do return false
		append(&project.storylet_groups, command.storylet_group)
	case .Storylet:
		for &item in project.storylets do if item.id == command.storylet.id {if !update do return false; item = command.storylet; return true}
		if update do return false
		append(&project.storylets, command.storylet)
	case .Ending:
		for &item in project.endings do if item.id == command.ending.id {if !update do return false; item = command.ending; return true}
		if update do return false
		append(&project.endings, command.ending)
	case .Invariant:
		for &item in project.invariants do if item.id == command.invariant.id {if !update do return false; item = command.invariant; return true}
		if update do return false
		append(&project.invariants, command.invariant)
	}
	return true
}
...
story_apply_command_raw :: proc(project: ^Story_Project, command: ^Story_Command) -> bool {
	switch command.kind {
	case .Add_Record:
		return story_apply_record(project, command, false)
	case .Update_Record:
		return story_apply_record(project, command, true)
	case .Add_Entity:
		append(&project.entities, command.entity); return true
	case .Add_Variable:
		append(&project.variables, command.variable); return true
	case .Add_Proposition:
		append(&project.propositions, command.proposition); return true
	case .Add_Scene:
		append(&project.scenes, command.scene); return true
	case .Add_Storylet:
		append(&project.storylets, command.storylet); return true
	case .Delete_By_Id:
...
story_command_batch :: proc(
	project: ^Story_Project,
	expected_revision: u64,
	commands: []Story_Command,
	dry_run: bool = false,
) -> Story_Command_Batch_Result {
	result := Story_Command_Batch_Result {
		revision = project.revision,
	}; if expected_revision !=
	   project.revision {result.stale = true; result.message = fmt.tprintf("stale revision: expected %d, current %d", expected_revision, project.revision); return result}
	working := story_project_clone(project)
	for &command in commands {if !story_apply_command_raw(
			&working,
			&command,
		) {story_project_destroy(&working)
			result.message = "command batch rejected without changes"
			return result}
		if result.changed_count <
		   len(
			   result.changed_ids,
		   ) {result.changed_ids[result.changed_count] = story_command_id(&command); result.changed_count += 1}}
	...
	result.ok =
		true; result.revision = working.revision; result.message = dry_run ? "dry run valid" : "command batch committed"; result.impact_summary = fmt.tprintf("%d record%s changed; validation has %d warning%s", result.changed_count, result.changed_count == 1 ? "" : "s", result.validation.warning_count, result.validation.warning_count == 1 ? "" : "s")
	if dry_run {story_project_destroy(&working); return result}
	story_project_destroy(project); project^ = working; return result
}

story_command_result_destroy :: proc(
	result: ^Story_Command_Batch_Result,
