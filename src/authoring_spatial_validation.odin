...
authoring_spatial_add :: proc(result:^[dynamic]Authoring_Diagnostic, doc:^Level_Document, entity, field:string, severity:Authoring_Diagnostic_Severity, message:string, story:int, position:Vec2) {
...
authoring_spatial_point_obstructed :: proc(doc:^Level_Document, story:int, position:Vec2, ignore_id:string="")->(string,string) {
...
authoring_spatial_marker_reference_compatible :: proc(project:^Story_Project,doc:^Level_Document, marker:Level_Marker)->bool {
	...
	case .Clue:
		payload:=mystery_payload(project);if payload!=nil&&mystery_clue_index(payload,marker.reference)>=0 do return true
		// A clue marker may bind the physical StoryCore source entity while the
		// MysteryDomain clue keeps its own stable record ID.
		return story_entity_index(project,marker.reference)>=0
	case .Camera,.Staging,.Player_Spawn,.Interaction,.Trigger,.Transition:return true
	}
	return true
}
...
authoring_spatial_validate_levels :: proc(levels:[]^Level_Document,project:^Story_Project=nil)->[dynamic]Authoring_Diagnostic {
	...
	for doc in levels {
		...
		for marker,index in doc.markers {
			...
			if marker.reference!="" {for other in doc.markers[index+1:] do if other.kind==marker.kind&&other.reference==marker.reference&&other.story!=marker.story do authoring_spatial_add(&result,doc,other.id,"reference",.Error,"typed marker reference is duplicated across story scopes",other.story,other.position)}
...
authoring_spatial_validate_level :: proc(doc:^Level_Document,project:^Story_Project=nil)->[dynamic]Authoring_Diagnostic {levels:=[1]^Level_Document{doc};return authoring_spatial_validate_levels(levels[:],project)}
