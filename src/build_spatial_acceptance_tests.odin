...
run_build_spatial_acceptance_tests :: proc() {
	...
	append(&graph_document.conditions,Story_Condition{id="build_condition",kind=.Always});graph_document.condition_count=len(graph_document.conditions);append(&graph_document.effects,Story_Effect{id="build_effect",kind=.Spatial_Command,spatial_command=.Set_Interaction,spatial_target={doc.id,"door_front"},world_enabled=true});graph_document.effect_count=len(graph_document.effects)
	interaction:=Level_Command{kind=.Set_Interaction,entity_id="door_front",interaction=.Door,interaction_prompt="ENTER THE AUTHORED ROOM",condition_id="build_condition",focused_scene=test_story_project.scenes[0].id,interaction_range=2.75,initially_active=true,locked=true,powered=false,effect_id_count=1};interaction.effect_ids[0]="build_effect"
	level_history={};assert(level_commit_transaction(&doc,interaction,"complete interaction"))
	text:=level_serialize(&doc);roundtrip_path:="/private/tmp/chicago-build-spatial-roundtrip.toml";assert(os.write_entire_file(roundtrip_path,transmute([]byte)text)==nil)
	...
	level_document=level_clone_document(&roundtrip);defer authoring_level_document_destroy(&level_document);runtime_game:=Game{};runtime_interactives_rebuild(&runtime_game);runtime_index:=runtime_interactive_index(&runtime_game,"door_front");assert(runtime_index>=0);runtime_item:=runtime_game.interactives[runtime_index];assert(runtime_item.prompt==interaction.interaction_prompt&&runtime_item.condition_id==interaction.condition_id&&runtime_item.focused_scene==interaction.focused_scene&&runtime_item.effect_id_count==1&&runtime_item.effect_ids[0]==interaction.effect_ids[0]&&runtime_item.interaction_range==2.75&&runtime_item.active&&runtime_item.locked&&!runtime_item.powered)

	// Default pickers remain scoped to one story; an explicit request is needed
	// to expose qualified records from another floor.
	append(&doc.stories,Level_Story{id="acceptance_upper",name="Acceptance Upper",base_elevation=3,wall_height=3});append(&doc.markers,Level_Marker{id="upper_camera",kind=.Camera,story=len(doc.stories)-1,position={2,2},camera_height=2})
	...
	handoff_game:=Game{};assert(graph_document.node_count>0);old_camera:=graph_document.nodes[0].beat.camera;graph_document.nodes[0].beat.camera="handoff_camera";graph_marker:=Level_Marker{id="handoff_camera",kind=.Camera};assert(editor_open_marker_in_graph(&handoff_game,graph_marker)&&handoff_game.editor_mode==.Graph&&graph_state.selected_node==0);graph_document.nodes[0].beat.camera=old_camera
	old_active:=active_story_project;active_story_project=test_story_project;payload:=mystery_payload(&active_story_project);assert(payload!=nil&&len(payload.clues)>0);mystery_marker:=Level_Marker{id="handoff_clue",kind=.Clue,reference=payload.clues[0].id};assert(editor_open_marker_in_mystery(&handoff_game,mystery_marker)&&handoff_game.screen==.Authoring&&authoring_workspace.tab==.Mystery&&authoring_workspace.selected_category==int(Mystery_Authoring_Record_Kind.Clue)&&authoring_mystery_id(.Clue,authoring_workspace.selected_record)==payload.clues[0].id);active_story_project=old_active
	...
	template_action_doc:=level_clone_document(&doc);template_story_count:=len(template_action_doc.stories);assert(level_reuse_action(&template_action_doc,.Create_Level_From_Template,"","instantiated_level","Instantiated Level",{})&&template_action_doc.id=="instantiated_level"&&template_action_doc.name=="Instantiated Level"&&len(template_action_doc.stories)==template_story_count);authoring_level_document_destroy(&template_action_doc)
	room_count:=len(doc.rooms);assert(level_reuse_action(&doc,.Instantiate_Room_Prefab,doc.rooms[0].id,"room_prefab_copy","Room Prefab",{.5,.5})&&len(doc.rooms)==room_count+1)
	object_count:=len(doc.objects);assert(level_reuse_action(&doc,.Instantiate_Prop_Prefab,doc.objects[0].id,"prop_prefab_copy","Prop Prefab",{6,6})&&len(doc.objects)==object_count+1)
}
