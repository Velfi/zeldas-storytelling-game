...
Compiled_Story :: struct {
	...
	nodes,
	storylets,
	endings: [dynamic]Compiled_Id,
	compiled_nodes:                                                                                                [dynamic]Compiled_Story_Node,
}

Story_Compile_Result :: struct {
	ok:         bool,
	message:    string,
...
story_hash_value :: proc(hash: u64, value: Story_Value) -> u64 {result :=
		(hash ~ u64(value.kind)) * 1099511628211
	result = (result ~ u64(value.boolean_value)) * 1099511628211
	result = (result ~ u64(value.integer_value)) * 1099511628211
	return story_hash_text(result, value.text_value)}
...
compiled_story_destroy :: proc(story: ^Compiled_Story) {
	...
	delete(
		story.entities,
	); delete(story.variables); delete(story.propositions); delete(story.events); delete(story.objectives); delete(story.conditions); delete(story.effects)
	delete(
		story.scenes,
	); delete(story.nodes); delete(story.storylets); delete(story.endings); delete(story.compiled_nodes)
	story^ = {}
}

story_compile_result_destroy :: proc(result: ^Story_Compile_Result) {compiled_story_destroy(
		&result.story,
	)
	story_validation_destroy(&result.validation)
	result^ = {}}

compile_story_project :: proc(source: ^Story_Project) -> Story_Compile_Result {
	result := Story_Compile_Result{}
	if source == nil {result.message = "story project is required"; return result}
	working := story_project_clone(source); defer story_project_destroy(&working)
	project := &working
	if resolved := story_resolve_environment(project);
	   !resolved.ok {result.message = resolved.message; return result}
	...
	compiled := Compiled_Story {
		...
		runtime         = story_project_clone(project),
	}
	...
	for mapping in compiled.nodes {
		...
		append(
			&compiled.compiled_nodes,
			Compiled_Story_Node {
				id = mapping.runtime_id,
				scene = compiled_id_find(compiled.scenes[:], node.scene_id),
				speaker = compiled_id_find(compiled.entities[:], node.speaker_id),
				next = compiled_id_find(compiled.nodes[:], node.next),
				success = compiled_id_find(compiled.nodes[:], node.success),
				failure = compiled_id_find(compiled.nodes[:], node.failure),
				cancel = compiled_id_find(compiled.nodes[:], node.cancel),
				subscene = compiled_id_find(compiled.scenes[:], node.subscene_id),
				line_id = node.line_id,
				text = node.text,
				kind = node.kind,
				condition_root = compiled_id_find(compiled.conditions[:], node.condition_id),
				first_effect = STORY_INVALID_RUNTIME_ID,
				effect_count = node.effect_id_count,
			},
		)
	}
	hash: u64 = 1469598103934665603
	hash = story_hash_text(
		hash,
		project.version,
	); hash = story_hash_text(hash, compiled.project_id); hash = story_hash_text(hash, project.title); hash = story_hash_text(hash, project.creator); hash = story_hash_text(hash, project.description); hash = story_hash_text(hash, compiled.content_version); hash = story_hash_text(hash, project.default_space_id)
	for requirement in project.expansion_requirements {record := story_hash_text(story_record_hash(), requirement.id); record = story_hash_text(record, requirement.version); record = (record ~ u64(requirement.optional)) * 1099511628211; record = (record ~ u64(requirement.distribution)) * 1099511628211; record = (record ~ u64(requirement.fallback)) * 1099511628211; hash ~= record}
	for capability in project.capabilities {record := story_hash_text(story_record_hash(), capability.id); record = story_hash_text(record, capability.version); hash ~= record}
	for resolved in project.resolved_environment.expansions {record := story_hash_text(story_record_hash(), resolved.id); record = story_hash_text(record, resolved.namespace); record = story_hash_text(record, resolved.version); record = story_hash_text(record, resolved.content_hash); hash ~= record}
	...
	for mapping in compiled.entities {source := story_entity_index(project, mapping.stable_id); if source >= 0 {item := project.entities[source]; hash = story_hash_text(hash, item.kind); hash = story_hash_text(hash, item.display_name); hash = story_hash_text(hash, item.description); hash = story_hash_text(hash, item.appearance_model_asset_ref); hash = story_hash_text(hash, item.default_animation); hash = story_hash_text(hash, item.visibility_condition_id); hash = story_hash_text(hash, item.availability_condition_id); hash = story_hash_text(hash, item.completion_condition_id); hash = story_hash_text(hash, item.interaction_scene_id); hash = story_hash_text(hash, item.interaction_prompt); hash = story_hash_text(hash, item.unavailable_prompt); hash = story_hash_text(hash, item.completion_prompt); hash = story_hash_text(hash, item.spatial.space_id); hash = story_hash_text(hash, item.spatial.target_id); hash = (hash ~ u64(item.spatial.target_kind)) * 1099511628211; hash = (hash ~ u64(item.volume)) * 1099511628211; hash = (hash ~ u64(item.container_capacity)) * 1099511628211; hash = (hash ~ u64(item.initially_locked)) * 1099511628211; hash = story_hash_text(hash, item.owner_id); hash = story_hash_text(hash, item.initial_container_id); for tag in item.tags[:item.tag_count] do hash = story_hash_text(hash, tag); for role in item.roles[:item.role_count] do hash = story_hash_text(hash, role)}}
	...
	for capability in project.capabilities {adapter := story_domain_find(capability.id, capability.version); if adapter != nil && adapter.hash != nil do hash ~= adapter.hash(project, story_record_hash())}
