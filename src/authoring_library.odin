...
Authoring_Artifact_Kind :: enum {Story, Campaign, Expansion}
Authoring_Package_Integrity :: enum {Unknown, Valid, Corrupt}
Authoring_Package_Compatibility :: enum {Compatible, Missing_Dependency, Incompatible, Unsupported_Format}
Authoring_Import_Decision :: enum {Coexist, Upgrade, Replace, Editable_Copy}
Authoring_Package_Warning_Audience :: enum {Player_Safe, Creator_Only}
Authoring_Package_Warning :: struct {message:string,audience:Authoring_Package_Warning_Audience}
...
Authoring_Package_File :: struct {path, sha256:string, size:u64, asset:bool}
Authoring_Library_Dependency_Edge :: struct {dependent, requirement:Authoring_Content_Identity, optional:bool}
...
authoring_library_installed_index :: proc(library: ^Authoring_Library, identity: Authoring_Content_Identity) -> int {
	for item, index in library.installed do if authoring_content_identity_equal(item.identity, identity) do return index
	return -1
}

authoring_library_installed_versions :: proc(library: ^Authoring_Library, kind: Authoring_Artifact_Kind, id: string) -> [dynamic]int {
	result: [dynamic]int
...
authoring_package_warning_add :: proc(inspection:^Authoring_Package_Inspection,message:string,audience:Authoring_Package_Warning_Audience) {append(&inspection.compatibility_warnings,message);append(&inspection.typed_warnings,Authoring_Package_Warning{message,audience})}
...
authoring_package_inspection_finalize :: proc(inspection: ^Authoring_Package_Inspection) {
	...
	case .Unknown: append(&inspection.diagnostics, authoring_diagnostic_init(.Packaging, "package", inspection.artifact.identity.id, "integrity", .Blocking, "package integrity has not been verified"))
	case .Corrupt: append(&inspection.diagnostics, authoring_diagnostic_init(.Packaging, "package", inspection.artifact.identity.id, "integrity", .Blocking, "package integrity verification failed"))
	case .Valid:
	}
	switch inspection.compatibility {
	case .Compatible:
	case .Missing_Dependency: append(&inspection.diagnostics, authoring_diagnostic_init(.Compatibility, "package", inspection.artifact.identity.id, "dependencies", .Blocking, "package dependencies are missing"))
	case .Incompatible: append(&inspection.diagnostics, authoring_diagnostic_init(.Compatibility, "package", inspection.artifact.identity.id, "capabilities", .Blocking, "package capabilities are incompatible"))
	case .Unsupported_Format: append(&inspection.diagnostics, authoring_diagnostic_init(.Compatibility, "package", inspection.artifact.identity.id, "format_version", .Blocking, "package format is unsupported"))
	}
}

authoring_package_inspection_installable :: proc(inspection: ^Authoring_Package_Inspection) -> bool {
...
authoring_library_plan_import :: proc(
	...
	plan := Authoring_Import_Plan{decision = decision, artifact = inspection.artifact, existing_index = -1, destination_root = destination_root}
	...
	versions := authoring_library_installed_versions(library, inspection.artifact.identity.kind, inspection.artifact.identity.id); defer delete(versions)
	switch decision {
	case .Coexist:
...
authoring_library_plan_uninstall :: proc(library: ^Authoring_Library, identity: Authoring_Content_Identity) -> Authoring_Uninstall_Plan {
	result := Authoring_Uninstall_Plan{installed_index = authoring_library_installed_index(library, identity)}
	if result.installed_index < 0 {
		append(&result.diagnostics, authoring_diagnostic_init(.Packaging, "library", identity.id, "", .Error, "installed version was not found"))
		return result
	}
	...
	for edge in library.dependency_edges {
		...
		alternate:=false;for candidate in library.installed do if !authoring_content_identity_equal(candidate.identity,identity)&&candidate.identity.kind==identity.kind&&candidate.identity.id==identity.id&&authoring_semver_compare(candidate.identity.content_version,edge.requirement.content_version)>=0 {alternate=true;break}
		if alternate do continue
		dependent:=authoring_library_installed_index(library,edge.dependent);if dependent>=0 {already:=false;for index in result.dependent_indices do if index==dependent do already=true;if !already do append(&result.dependent_indices,dependent)}
	}
...
authoring_plan_export :: proc(request: Authoring_Export_Request) -> Authoring_Export_Plan {
	result := Authoring_Export_Plan{identity = request.source.identity, destination_path = request.destination_path}
	...
	if request.validation == nil {
		append(&result.diagnostics, authoring_diagnostic_init(.Packaging, "source-project", request.source.identity.id, "validation", .Blocking, "export validation snapshot is required"))
	} else {
	...
	if request.assets == nil {
		append(&result.diagnostics, authoring_diagnostic_init(.Assets, "source-project", request.source.identity.id, "assets", .Blocking, "asset registry is required"))
	} else {
		...
		asset_diagnostics := project_asset_registry_diagnostics(request.assets); defer delete(asset_diagnostics)
