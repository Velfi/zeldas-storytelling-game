...
authoring_acceptance_fail :: proc(
	result: ^Authoring_Acceptance_Result,
	phase, message: string,
) -> Authoring_Acceptance_Result {
	result.phase = phase
...
run_authoring_acceptance :: proc(
	repository_root: string,
	keep_root := false,
) -> Authoring_Acceptance_Result {
	authoring_acceptance_counter += 1
	root := fmt.tprintf(
		"/tmp/chicago-authoring-acceptance-%d-%d",
		os.get_pid(),
		authoring_acceptance_counter,
	)
	result := Authoring_Acceptance_Result {
		phase = "setup",
		root  = root,
	...
	created := authoring_workspace_action_create_project_case(
		root,
		"acceptance_campaign",
		"Acceptance Campaign",
		"acceptance_mystery",
	...
	story_commands := [7]Story_Authoring_Command {
		{
			...
			variable = {
				id = "evidence_ready",
				display_name = "Evidence ready",
				kind = .Boolean,
				default_value = story_value_boolean(true),
			},
		},
		{
			kind = .Add,
			...
			condition = {
				id = "evidence_ready_cond",
				kind = .Value_Equals,
				variable_id = "evidence_ready",
		...
		},
		{
			...
			record_kind = .Entity,
			entity = {
				id = "acceptance_character_a",
				kind = "person",
				display_name = "Acceptance Character A",
			},
		},
		{
			kind = .Add,
			record_kind = .Entity,
			entity = {
				id = "acceptance_character_b",
				kind = "person",
				display_name = "Acceptance Character B",
			},
		},
		{
			kind = .Add,
			...
			proposition = {
				id = "acceptance_proposition",
				text = "The acceptance evidence identifies the culprit.",
			},
		},
		{
			kind = .Add,
			...
			fact = {
				id = "acceptance_fact",
				display_name = "Acceptance Fact",
				proposition = "acceptance_proposition",
		...
		},
		{
			...
			ending = {
				...
				title = "Case Solved",
				summary = "The contradiction closes the case.",
				priority = 1,
			},
		},
	}
	story_authored := authoring_workspace_action_apply_story_commands(
		&story,
		&story_history,
		story_commands[:],
	); defer story_authoring_result_destroy(&story_authored); if !authoring_acceptance_require(&result, story_authored.ok) do return authoring_acceptance_fail(&result, "story", story_authored.message)
	choice := Story_Node {
		id           = "conditional_conversation",
		scene_id     = "opening",
		kind         = .Choice,
		ui           = "dialogue",
		camera       = "evidence_camera",
		actor        = "culprit",
		actor_mark   = "evidence_staging",
		domain_ref   = "evidence_interaction_marker",
		choice_count = 1,
	}
	choice.choices[0] = {
		id           = "proceed",
		...
		target       = "opening_end",
		condition_id = "evidence_ready_cond",
	}
	...
	claim := Mystery_Claim {
		...
		protects        = "culprit",
		response        = "The evidence is misleading.",
	...
	deduction := Mystery_Deduction {
		...
		category       = "support",
		support_count  = 1,
	}; deduction.supports[0] = "initial_clue"
	question := Mystery_Question {
		id                      = "culprit_question",
		prompt                  = "Who is protected by the evidence?",
		hypothesis_id           = "case_support",
		category                = "final",
		require_deduction_count = 1,
		required_for_final      = true,
	}; question.requires_deductions[0] = "evidence_deduction"
	dialogue := Mystery_Dialogue_Metadata {
		node_id       = "conditional_conversation",
		character_id  = "culprit",
		prompt        = "Present the evidence",
		response      = "You cannot prove it.",
		clue_id       = "initial_clue",
		interaction   = "evidence_interaction_marker",
		require_count = 1,
	}; dialogue.requires[0] = "initial_clue"
	contradiction := Mystery_Contradiction {
		id            = "culprit_contradiction",
		claim_id      = "culprit_claim",
		fact_id       = "acceptance_fact",
		conclusion_id = "case_support",
		explanation   = "The authored fact contradicts the culprit's claim.",
	}
	demonstration := Mystery_Demonstration {
		id             = "culprit_demonstration",
		question_id    = "culprit_question",
		mode           = "evidence",
		resolution     = "supported",
		result         = "proven",
		prompt         = "Place the decisive evidence.",
		slot_count     = 1,
		accepted_count = 1,
		route_count    = 1,
		result_count   = 1,
	}; demonstration.slot_labels[0] = "Evidence"; demonstration.slot_types[0] = "clue"; demonstration.accepted[0] = "initial_clue"; demonstration.route_firsts[0] = 0; demonstration.route_counts[0] = 1; demonstration.result_deductions[0] = "evidence_deduction"
	ending := Mystery_Ending_Metadata {
		ending_id      = "acceptance_ending",
		trigger        = "airtight",
		outcome        = "case solved",
		subtitle       = "Evidence accepted",
		epilogue       = "The contradiction closes the case.",
		tone           = "decisive",
		primary_label  = "Continue",
		primary_action = "campaign",
	}
	acceptance_clue := Mystery_Clue {
		id             = "acceptance_clue",
		source_id      = "acceptance_character_a",
		description    = "The acceptance clue identifies the culprit.",
		proposition_id = "acceptance_proposition",
	}
	acceptance_solution :=
	...
	append(
		...
		Mystery_Authoring_Command {
			...
			character = {
				entity_id = "acceptance_character_a",
				private_secret = "Witnessed the decisive evidence.",
				motive = "Tell the truth",
		...
		Mystery_Authoring_Command {
			...
			record_kind = .Character,
			character = {
				entity_id = "acceptance_character_b",
				private_secret = "Was elsewhere.",
				motive = "Avoid suspicion",
	...
	loaded_level := authoring_workspace_action_create_level(
		level_path,
		story.id,
		story.title,
		&level,
	...
	foundation_command := Level_Command {
		kind        = .Create_Foundation,
		entity_id   = "evidence_foundation",
		c           = {f32(Level_Foundation_Kind.Slab), .25},
		point_count = 4,
	}; foundation_command.points[0] = {
		2,
		2,
	}; foundation_command.points[1] = {12, 2}; foundation_command.points[2] = {12, 12}; foundation_command.points[3] = {2, 12}; append(&build_commands, foundation_command)
	room_command := Level_Command {
		kind        = .Create_Room_Polygon,
		entity_id   = "evidence_room",
		material    = "wood",
		point_count = 4,
	}; room_command.points[0] = {
		2.25,
		2.25,
	}; room_command.points[1] = {11.75, 2.25}; room_command.points[2] = {11.75, 11.75}; room_command.points[3] = {2.25, 11.75}; append(&build_commands, room_command)
	marker_specs := [8]struct {
		id, reference:          string,
		kind:                   Level_Marker_Kind,
		position:               Vec2,
		radius, facing, height: f32,
	} {
		{"culprit_spawn", "culprit", .Character_Spawn, {6, 8}, .5, 0, 0},
		{"suspect_a_spawn", "suspect_a", .Character_Spawn, {8, 8}, .5, 0, 0},
		{"suspect_b_spawn", "suspect_b", .Character_Spawn, {10, 8}, .5, 0, 0},
		{"evidence_spawn", "evidence_source", .Clue, {8, 5}, .5, 0, 0},
		{"evidence_interaction_marker", "initial_clue", .Interaction, {8, 5.75}, 1.25, 0, 0},
		{"evidence_trigger", trigger_reference, .Trigger, {7, 5}, 1.25, 0, 0},
		{"evidence_camera", "", .Camera, {8, 10}, .5, 270, 2.2},
		{"evidence_staging", "", .Staging, {9, 7}, .5, 180, 0},
	}
	...
	append(
		...
		Level_Command {
			kind = .Place_Object,
			entity_id = "evidence_interaction",
			a = {8, 5},
			material = "table",
		},
		Level_Command {
			...
			entity_id = "evidence_interaction",
			interaction = .Toggle,
			interaction_prompt = "Inspect evidence",
			interaction_range = 2,
			initially_active = true,
			powered = true,
		},
	)
	built := authoring_workspace_action_apply_level_commands(
		&level,
		build_commands[:],
	); if !authoring_acceptance_require(&result, built.ok) do return authoring_acceptance_fail(&result, "build", built.message)
	...
	provenance := Project_Asset_Provenance {
		source_uri               = "https://kenney.nl/assets",
		source_name              = "Bundled acceptance fixture",
		creator                  = "Kenney",
	...
	fixtures := [3]struct {
		source, destination, id: string,
		kind:                    Project_Asset_Kind,
	} {
		{
			"assets/ui/campaigns/unsorted-cases-hero.png",
			"assets/thumbnail",
			"acceptance_thumbnail",
			.Thumbnail,
		},
		{
	...
	for fixture in fixtures {
		...
		imported := project_asset_import(
			&assets,
			{
				project_root = root,
				source_path = source,
				destination_directory = fixture.destination,
				requested_id = fixture.id,
				kind = fixture.kind,
				mode = .Copy,
				embed_policy = .Embed,
	...
	object_index := level_object_index(
		&level,
		"evidence_interaction",
	); node_index := project_asset_story_node_index(&story, "opening"); if node_index < 0 && len(story.nodes) > 0 do node_index = 0
	...
	prop_mapping := authoring_workspace_action_map_asset(
		&assets,
		"acceptance_prop",
		.Prop_Model,
		"evidence_interaction",
		level = &level,
	); audio_mapping := authoring_workspace_action_map_asset(&assets, "acceptance_audio", .Sound_Cue, story.nodes[node_index].id, story = &story)
	...
	saved_campaign_workspace := new(
		Campaign_Workspace_State,
	); saved_campaign_workspace^ = campaign_workspace; campaign_workspace = {
		draft = {},
	}
	campaign_actions := [3]Campaign_Workspace_Action {
		{
			kind = .Set_Metadata,
			metadata = {
				version = "MysteryCampaign v2",
				id = "acceptance_campaign",
				title = "Acceptance Campaign",
				creator = "Acceptance Harness",
				description = "Clean-profile acceptance",
				content_version = "2.0.0",
			},
		},
		{kind = .Add_Condition, condition = {kind = .Always}},
		{
			kind = .Add_Case,
			case_value = {
				id = "acceptance_mystery",
				title = "Acceptance Mystery",
				story_path = item.paths.story,
				level_path = item.paths.level,
	...
	campaign_authored := campaign_workspace_apply_actions(
		campaign_actions[:],
	); if !authoring_acceptance_require(&result, campaign_authored.ok && campaign_workspace.dirty && campaign_workspace.history.undo_count == 1) do return authoring_acceptance_fail(&result, "campaign", campaign_authored.message)
	campaign := campaign_clone(
		&campaign_workspace.draft,
	); defer campaign_destroy(&campaign); campaign_destroy(&campaign_workspace.draft); campaign_workspace = saved_campaign_workspace^; free(saved_campaign_workspace)
	thumbnail_mapping := authoring_workspace_action_map_asset(
		&assets,
		"acceptance_thumbnail",
		.Campaign_Thumbnail,
		campaign.id,
		campaign = &campaign,
	); if !authoring_acceptance_require(&result, thumbnail_mapping.ok) do return authoring_acceptance_fail(&result, "assets", "production typed asset mapper did not author campaign thumbnail")
	...
	config_text := fmt.tprintf(
		"{{\"author\":\"Acceptance Harness\",\"description\":\"Clean-profile acceptance\",\"content_version\":\"1.0.0\",\"story\":\"%s\",\"level\":\"%s\",\"thumbnail\":\"assets/thumbnail/unsorted-cases-hero.png\",\"include\":[\"assets\"],\"acknowledge_incomplete_validation\":true}}",
		item.paths.story,
		item.paths.level,
	)
	...
	saved_validation_level :=
		level_document; saved_validation_story := graph_source_project; saved_validation_graph := graph_document; level_document = committed_snapshot.level; graph_source_project = &committed_snapshot.story; graph_document = committed_snapshot.graph
	export_validation := authoring_production_validate(
		.Exportable,
		&committed_snapshot.story,
		&graph_document,
		&committed_snapshot.level,
		nil,
		&assets,
		nil,
	); committed_snapshot.graph = graph_document; level_document = saved_validation_level; graph_source_project = saved_validation_story; graph_document = saved_validation_graph; defer authoring_validation_snapshot_destroy(&export_validation)
	if authoring_validation_is_blocked(
		&export_validation,
	) {detail := "production exportable validation blocked"; if len(export_validation.diagnostics) > 0 {issue := export_validation.diagnostics[0]; detail = fmt.tprintf("%v/%s/%s: %s", issue.domain, issue.entity_id, issue.field_path, issue.message)}; return authoring_acceptance_fail(&result, "export-validation", detail)}
	exported := authoring_workspace_action_staged_export(
		story.title,
		story.content_version,
		len(story.expansion_requirements),
		{
			kind = .Story,
			source_root = root,
	...
	authoring_workspace =
		{}; inspected := authoring_workspace_action_inspect(package_path, .Story, repository_root); if !authoring_acceptance_require(&result, inspected.ok && authoring_workspace.inspection.artifact.identity.content_version == "1.0.0") do return authoring_acceptance_fail(&result, "export", inspected.message)
	...
	installed_assets := [4]string {
		"assets/acceptance_thumbnail.png",
		"assets/acceptance_prop.glb",
		"assets/acceptance_audio.ogg",
		"assets/asset-attribution.toml",
	}
	...
	attribution_path := authoring_acceptance_join(
		installed.installed.install_root,
		"assets/asset-attribution.toml",
	)
	attribution_bytes, attribution_error := os.read_entire_file_from_path(
		attribution_path,
	...
	installed_story_path := authoring_acceptance_join(
		installed.installed.install_root,
		item.paths.story,
	); installed_level_path := authoring_acceptance_join(installed.installed.install_root, item.paths.level); installed_story: Story_Project; installed_level: Level_Document
	...
	after_export_story_hash, after_export_story_hashed := project_asset_sha256_file(
		story_path,
	); after_export_level_hash, after_export_level_hashed := project_asset_sha256_file(level_path); if !authoring_acceptance_require(&result, after_export_story_hashed.ok && after_export_level_hashed.ok && after_export_story_hash == committed_story_hash && after_export_level_hash == committed_level_hash) do return authoring_acceptance_fail(&result, "snapshot", "export did not consume the immutable committed snapshot")
	inspected = authoring_workspace_action_inspect(
		campaign_package,
		.Campaign,
		repository_root,
	); if !authoring_acceptance_require(&result, inspected.ok && authoring_workspace.inspection.case_count == 1) do return authoring_acceptance_fail(&result, "campaign-export", inspected.message)
	campaign_library := authoring_acceptance_join(root, "campaign-library")
	campaign_installed := authoring_workspace_action_install(
		campaign_library,
		.Coexist,
		"",
		repository_root,
	...
	snapshot := authoring_production_validate(
		.Draft,
		&story,
		&graph,
		&level,
	...
	asset_probe_index := len(
		authoring_workspace.assets.assets,
	); append(&authoring_workspace.assets.assets, Project_Asset_Record{id = "acceptance_navigation_asset"}); defer ordered_remove(&authoring_workspace.assets.assets, asset_probe_index)
	...
	probes := [8]Authoring_Diagnostic {
		...
		authoring_diagnostic_init(
			...
			"description",
			.Blocking,
			"acceptance Mystery focus probe",
		),
		authoring_diagnostic_init(
			.Graph,
			"graph",
			"conditional_conversation",
			"camera",
			.Blocking,
			"acceptance Graph focus probe",
		),
		authoring_diagnostic_init(
			.Level,
			"level",
		...
		authoring_diagnostic_init(
			...
			"condition_root",
			.Blocking,
			"acceptance Campaign focus probe",
		),
		authoring_diagnostic_init(
			.Assets,
			"assets",
			"acceptance_navigation_asset",
			"provenance.license_id",
			.Blocking,
			"acceptance Asset focus probe",
		),
		authoring_diagnostic_init(
			.Packaging,
			"package",
			...
			"destination",
			.Blocking,
			"acceptance Packaging focus probe",
		),
		authoring_diagnostic_init(
			.Compatibility,
			"package",
	...
	authoring_workspace.scenario_record = authoring_scenario_record_init(
		"acceptance_failure",
		"Acceptance failure trace",
	)
	defer authoring_scenario_record_destroy(&authoring_workspace.scenario_record)
	_ = authoring_scenario_record_action(
		&authoring_workspace.scenario_record,
		{action = "start", value = "missing_acceptance_scene"},
	)
	failure_context: Scenario_Context
	failure_story := authoring_acceptance_join(
		repository_root,
		"assets/stories/mysteries/the_torn_appointment.story.toml",
	); failure_level := authoring_acceptance_join(repository_root, "assets/levels/vale_house.toml")
	...
	authoring_workspace.scenario_last = authoring_scenario_replay(
		&authoring_workspace.scenario_record,
		&failure_context,
	); scenario_context_destroy(&failure_context)
	failure_path := authoring_acceptance_join(root, "scenario-failure.toml")
	failure_message := authoring_workspace_export_failure(
		failure_path,
	); failure_bytes, failure_error := os.read_entire_file_from_path(failure_path, context.temp_allocator)
	...
	append(
		&creator_setup.variables,
		Authoring_Creator_Variable{id = "evidence_ready", value = story_value_boolean(true)},
	)
	append(
		&creator_setup.knowledge,
		Authoring_Creator_Knowledge{kind = .Clue, id = "initial_clue", present = true},
	)
	playthrough := Campaign_Playthrough {
		campaign_id              = campaign.id,
		campaign_content_version = campaign.content_version,
		id                       = "acceptance-playthrough",
		name                     = "Acceptance",
	}
	playtest := new(Authoring_Playtest_Coordinator); defer free(playtest)
	saved_validation_level =
		level_document; saved_validation_story = graph_source_project; saved_validation_graph = graph_document; level_document = committed_snapshot.level; graph_source_project = &committed_snapshot.story; graph_document = committed_snapshot.graph
	started_playtest := authoring_playtest_begin(
		playtest,
		&committed_snapshot.story,
		&runtime,
		&committed_snapshot.graph,
		&committed_snapshot.level,
		&campaign,
		&playthrough,
		&assets,
	...
	committed_snapshot.graph =
		graph_document; level_document = saved_validation_level; graph_source_project = saved_validation_story; graph_document = saved_validation_graph
	if !authoring_acceptance_require(
		&result,
		started_playtest.ok && playtest.active,
	) {detail := started_playtest.message; if len(playtest.validation.diagnostics) > 0 do detail = fmt.tprintf("%s: %s", detail, playtest.validation.diagnostics[0].message); return authoring_acceptance_fail(&result, "selected-state", detail)}
