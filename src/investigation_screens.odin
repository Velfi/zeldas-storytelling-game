...
editor_advance_from_foundation :: proc(
	g: ^Game,
	doc: ^Level_Document,
	foundation_index: int,
) -> bool {if foundation_index < 0 || foundation_index >= len(doc.foundations) do return false
	created := doc.foundations[foundation_index]
	if created.kind == .Basement && created.story >= 0 do _ = level_set_active_story(doc, created.story)
	g.build_tool = .Room
	editor_state.room_mode = .Rectangle
	editor_state.selection[0] = {.Foundation, created.id, -1}
	editor_state.selection_count = 1
	return true}

editor_create_room_from_foundation :: proc(
	g: ^Game,
	doc: ^Level_Document,
	foundation_id: string,
) -> bool {index := level_foundation_index(doc, foundation_id); if index < 0 do return false
	foundation := doc.foundations[index]
	story := foundation.kind == .Basement ? foundation.story : level_ground_story(doc)
	if story < 0 do return false
	if doc.active_story != story do _ = level_set_active_story(doc, story)
	room_id := level_next_id("room", doc.revision)
	create := Level_Command {
		kind        = .Create_Room_Polygon,
		entity_id   = room_id,
		material    = strings.to_lower(fmt.tprintf("%v", g.build_surface)),
		point_count = min(len(foundation.points), 32),
	}
	for point, i in foundation.points {if i >= create.point_count do break; create.points[i] = point}
	preview := create
	preview.entity_id = ""
	if level_preview_transaction(doc, preview).state == .Blocked do return false
	commands := [2]Level_Command {
		create,
		{
			kind = .Set_Platform,
			entity_id = room_id,
	...
	if !level_commit_transactions(doc, commands[:], "Create room from foundation") do return false
	editor_state.selection[0] = {.Room, room_id, -1}
	editor_state.selection_count = 1
	g.build_tool = .Select
	return true}

editor_finish_foundation_polygon :: proc(
	g: ^Game,
	repeat: bool,
) -> bool {if editor_state.foundation_draw_count < 3 do return false; command := Level_Command {
		kind        = .Create_Foundation,
		value       = editor_state.foundation_elevation,
		c           = {f32(editor_state.foundation_kind), editor_state.foundation_depth},
		point_count = editor_state.foundation_draw_count,
	}; copy(
		command.points[:],
		editor_state.foundation_draw_points[:editor_state.foundation_draw_count],
...
update_campaign_workspace :: proc(g: ^Game) {
	...
	if button(
		g,
		{178, 666, 150, 38},
	) {saved := save_campaign_manifest(campaign_manifest_path, doc); campaign_workspace.feedback = saved.message; campaign_workspace.diagnostics = saved}
	if button(
		g,
		{338, 666, 150, 38},
	) {campaign_workspace.diagnostics = campaign_validate(doc); campaign_workspace.feedback = campaign_workspace.diagnostics.message}
	...
	case .Overview:
		if button(g, {40, 120, 500, 34}) do campaign_workspace_begin_text(.Campaign_Format, doc.version)
		if button(g, {550, 120, 500, 34}) do campaign_workspace_begin_text(.Campaign_ID, doc.id)
		if button(g, {40, 164, 500, 34}) do campaign_workspace_begin_text(.Campaign_Title, doc.title)
		if button(g, {550, 164, 500, 34}) do campaign_workspace_begin_text(.Campaign_Creator, doc.creator)
		if button(g, {40, 208, 500, 34}) do campaign_workspace_begin_text(.Campaign_Version, doc.content_version)
		if button(g, {550, 208, 500, 34}) do campaign_workspace_begin_text(.Campaign_Thumbnail, doc.thumbnail)
		if button(g, {40, 252, 1010, 120}) do campaign_workspace_begin_text(.Campaign_Description, doc.description)
		if button(
			g,
			{40, 390, 150, 36},
		) {path := authoring_native_save_file("New Campaign", "campaign.toml")
			if path != "" do campaign_workspace.feedback = campaign_workspace_new_manifest(path).message}
		if button(g, {200, 390, 150, 36}) {path := authoring_native_open_file("Open Campaign")
			if path != "" do campaign_workspace.feedback = campaign_workspace_open_manifest(path).message}
		if button(
			g,
			{360, 390, 150, 36},
		) {path := authoring_native_save_file("Duplicate Campaign", fmt.tprintf("%s-copy.toml", doc.id))
			if path != "" do campaign_workspace.feedback = campaign_workspace_save_as(path, true).message}
		if button(
			g,
			{520, 390, 150, 36},
		) {path := authoring_native_save_file("Save Campaign As", fmt.tprintf("%s.toml", doc.id))
			if path != "" do campaign_workspace.feedback = campaign_workspace_save_as(path).message}
		if button(
			g,
			{680, 390, 150, 36},
		) {path := authoring_native_save_file("Move Campaign", fmt.tprintf("%s.toml", doc.id))
			if path != "" do campaign_workspace.feedback = campaign_workspace_move_manifest(path).message}
		if button(
			g,
			{840, 390, 190, 36},
	...
	case .Cases:
		if button(g, {30, 108, 120, 32}) do _ = campaign_workspace_add_case()
		if button(g, {158, 108, 80, 32}) do _ = campaign_workspace_move_case(-1)
		if button(g, {246, 108, 80, 32}) do _ = campaign_workspace_move_case(1)
		if button(g, {334, 108, 110, 32}) do _ = campaign_workspace_remove_case()
	...
	case .Conditions:
		...
		kinds := [11]Campaign_Condition_Kind {
			.Always,
			.Never,
			.All,
			.Any,
	...
	case .Effects:
		...
		if campaign_workspace.selected_case >= 0 &&
		   campaign_workspace.selected_case <
			...
			for offset in 0 ..< count {effect_index := first + offset; if button(g, {500, 164 + f32(offset) * 34, 450, 28}) do campaign_workspace.selected_effect = effect_index}}
		   ...
		   campaign_workspace.selected_effect <
			...
			if len(doc.variables) > 0 &&
			   button(
				   g,
				   {500, 500, 450, 38},
			   ) {current := campaign_variable_index(doc, effect.variable_id)
				current = (current + 1) % len(doc.variables)
				variable := doc.variables[current]
				effect.variable_id = variable.id
				effect.kind =
					variable.kind == .Boolean ? .Set_Boolean : variable.kind == .Integer ? .Set_Integer : .Set_Enum
				effect.enum_value = variable.default_enum
				campaign_workspace_mark_changed("EFFECT TARGET UPDATED")}
			if effect.kind == .Set_Boolean &&
			   button(g, {500, 548, 220, 38}) {effect.boolean_value = !effect.boolean_value
				campaign_workspace_mark_changed("EFFECT VALUE UPDATED")}
			if effect.kind == .Set_Integer ||
			   effect.kind ==
				   .Add_Integer {if button(g, {500, 548, 100, 38}) {effect.integer_value -= 1; campaign_workspace_mark_changed("EFFECT VALUE UPDATED")}
				if button(g, {610, 548, 100, 38}) {effect.integer_value += 1
					campaign_workspace_mark_changed("EFFECT VALUE UPDATED")}
				if button(
					g,
					{720, 548, 230, 38},
				) {effect.kind = effect.kind == .Set_Integer ? .Add_Integer : .Set_Integer
					campaign_workspace_mark_changed("EFFECT OPERATION UPDATED")}}
			if effect.kind ==
			   .Set_Enum {variable := campaign_variable_index(doc, effect.variable_id)
				if variable >= 0 && button(g, {500, 548, 450, 38}) {v := doc.variables[variable]
					if v.enum_value_count >
					   0 {current := 0; for j in 0 ..< v.enum_value_count do if v.enum_values[j] == effect.enum_value do current = j
						effect.enum_value = v.enum_values[(current + 1) % v.enum_value_count]
						campaign_workspace_mark_changed("EFFECT VALUE UPDATED")}}}}
		if button(g, {500, 600, 150, 34}) do _ = campaign_workspace_move_effect_mapping(-1)
		if button(g, {660, 600, 150, 34}) do _ = campaign_workspace_move_effect_mapping(1)
		if button(g, {820, 600, 190, 34}) do _ = campaign_workspace_remove_effect_mapping()
	case .Simulation:
		if len(doc.variables) >
		   0 {if button(g, {780, 108, 180, 34}) do campaign_workspace.selected_variable = (campaign_workspace.selected_variable + 1) % len(doc.variables)
			v := doc.variables[campaign_workspace.selected_variable]
			if v.kind == .Boolean && button(g, {970, 108, 180, 34}) do campaign_workspace.feedback = campaign_workspace_simulation_change_variable(campaign_workspace.selected_variable, 1).message
			else if v.kind == .Integer {if button(g, {970, 108, 82, 34}) do campaign_workspace.feedback = campaign_workspace_simulation_change_variable(campaign_workspace.selected_variable, -1).message; if button(g, {1060, 108, 82, 34}) do campaign_workspace.feedback = campaign_workspace_simulation_change_variable(campaign_workspace.selected_variable, 1).message} else if v.enum_value_count > 0 && button(g, {970, 108, 180, 34}) do campaign_workspace.feedback = campaign_workspace_simulation_change_variable(campaign_workspace.selected_variable, 1).message}
	...
	case .Diagnostics:
		if button(g, {30, 130, 240, 38}) do campaign_workspace.diagnostics = campaign_validate(doc)
		if button(g, {30, 180, 1120, 36}) do _ = campaign_workspace_navigate_validation()
		for _, i in doc.cases do if button(g, {40, 260 + f32(i) * 34, 1080, 30}) do _ = campaign_workspace_navigate_dependency(i)
	}
}
...
discover_clue_free :: proc(g: ^Game, clue_index: int) -> bool {
	...
	if first_examination &&
	   g.screen ==
		   .Investigate {context_feedback(g, "OBSERVATION RECORDED · OPEN NOTEBOOK", .Complete, "notebook_recorded"); g.context_ui.feedback_expires = g.animation_time + 5}
	return true
}

case_uses_city_tutorial :: proc(g: ^Game) -> bool {payload := mystery_game_payload(g); return(
		payload != nil &&
		payload.tutorial_id == "basic_controls" \
	)}
case_starts_in_city :: proc(g: ^Game) -> bool {payload := mystery_game_payload(g); return(
		payload != nil &&
		payload.city_start != "" \
	)}
...
advance_introduction :: proc(g: ^Game) {
	...
	case 1:
		...
		refresh_questions(
			g,
		); g.question_feedback = "Begin with what looks out of place. Statements and observations will open questions on your board."
...
reset_case_state :: proc(g: ^Game, story_seed: u64, destination: Screen) {
	...
	gui :=
		g.gui; audio_stream := g.audio_stream; vehicle_audio_stream := g.vehicle_audio_stream; sounds := g.sounds; mute := g.mute; aa_mode := g.aa_mode; lighting_quality := g.lighting_quality; guidance_mode := g.guidance_mode; tutorial := g.tutorial; persist_seed := g.persist_seed; active_device := g.active_device
	story_project :=
		g.story_project; compiled_story := g.compiled_story; story_runtime := g.story_runtime; spatial_service := g.spatial_service
	gui.focused =
		destination == .Title ? button_id({410, 400, 380, 48}) : button_id({410, 625, 380, 58})
	...
	g^ = Game {
		...
		vehicle_audio_stream      = vehicle_audio_stream,
		sounds                    = sounds,
		player_x                  = 3.5,
		player_y                  = 12.5,
		camera_x                  = 3.5,
	...
	if story_runtime != nil &&
	   compiled_story !=
		   nil {story_runtime_destroy(story_runtime); story_runtime^ = story_runtime_new(compiled_story, spatial_service)}
	g.story_project =
		story_project; g.compiled_story = compiled_story; g.story_runtime = story_runtime; g.story_state = story_runtime == nil ? nil : &story_runtime.state; g.mystery_state = story_runtime == nil ? nil : cast(^Mystery_State)story_runtime_capability_state(story_runtime, "mystery", MYSTERY_DOMAIN_VERSION); g.spatial_service = spatial_service
	g.guidance_mode = guidance_mode; g.tutorial = tutorial; g.tutorial.guidance = guidance_mode
	if !case_uses_city_tutorial(
		g,
	) {tutorial_complete(g, .Move); tutorial_complete(g, .Look); tutorial_complete(g, .Briefing); tutorial_complete(g, .Travel)}
	payload := mystery_game_payload(
		g,
	); if destination == .Exterior && payload != nil && payload.city_start != "" do _ = city_place_at_landmark(g, payload.city_start)
	if destination == .Investigate do _ = apply_player_spawn_marker(g)
	initialize_dispositions(g); initialize_character_animations(g)
}
enter_game_over :: proc(g: ^Game, reason: string) {g.game_over_reason = reason
	g.screen = .Game_Over
	g.gui.focused = button_id({410, 410, 380, 52})
	g.focus_screen_initialized = false}
route_locked_investigation :: proc(g: ^Game) {if g.game_over_reason != "" {if !enter_case_ending(g, "out_of_time") do g.screen = .Game_Over}
	else {g.screen = .Reveal_Prep}}

active_case_ending :: proc(g: ^Game) -> ^Mystery_Ending_Metadata {payload := mystery_game_payload(
		g,
	)
	if payload == nil || g.active_ending < 0 || g.active_ending >= len(payload.endings) do return nil
	return &payload.endings[g.active_ending]}
case_ending_trigger_for_outcome :: proc(outcome: Outcome) -> string {return fmt.tprintf(
		"outcome.%s",
		campaign_outcome_text(outcome),
	)}
enter_case_ending :: proc(g: ^Game, id_or_trigger: string) -> bool {
	...
	g.active_ending =
		index; ending := active_case_ending(g); if ending == nil do return false; g.show_canonical = false; g.phase = .Case_Result; g.screen = .Result; g.focus_screen_initialized = false
	if outcome, ok := campaign_outcome_from_text(ending.outcome);
	   ok {g.result = outcome; if g.persist_seed {applied := campaign_apply_result(&campaign_document, &campaign_playthrough, Campaign_Case_Result{case_id = g.story_project.id, outcome = outcome}); if applied.ok do _ = save_campaign_playthrough()}}
	return true
}
perform_case_ending_action :: proc(g: ^Game, action: string) {
	switch action {case "campaign":
		reset_case_state(g, begin_new_story_seed(), .Campaign); case "restart":
		destination: Screen = case_starts_in_city(g) ? .Exterior : .Investigate
		reset_case_state(g, g.run_seed, destination); case "title":
		reset_case_state(g, begin_new_story_seed(), .Title); case "quit":
		g.running = false; case "reveal":
		g.show_canonical = true}
}
...
campaign_prepare_case :: proc(index: int, out: ^Case_Load_Work) -> Validation {
	...
	compiled := compile_story_project(
		&out.story,
	); if !compiled.ok do return {false, compiled.message}; out.compiled = compiled.story
	...
	assert(
		story_spatial_registry_register(&out.spatial_registry, story_level_space(&out.level)),
	); _ = story_spatial_registry_register(&out.spatial_registry, story_city_space()); bindings := story_spatial_validate_project(&out.story, &out.spatial_registry); if !bindings.ok {story_validation_destroy(&bindings); return {false, "story has invalid spatial bindings"}}; story_validation_destroy(&bindings)
...
campaign_case_load_worker :: proc(_: ^thread.Thread) {
	...
	sync.atomic_store_explicit(
		&case_load_work.state,
		case_load_work.result.ok ? i32(5) : i32(6),
		.Release,
	)
}

campaign_case_load_reap :: proc() {if case_load_work.worker != nil {thread.join(
			case_load_work.worker,
		)
		thread.destroy(case_load_work.worker)
		case_load_work.worker = nil}}
...
authoring_app_request_build_exit :: proc(g: ^Game) {
	...
	if active_authoring_ready &&
	   ...
	   ) {editor_state.exit_confirm_visible = true; editor_state.shortcut_help_visible = false} else {g.editor_mode = .None; g.interactive_count = 0}
...
campaign_finish_prepared_case :: proc(g: ^Game) -> Validation {
	...
	); compiled_story_destroy(&active_compiled_story); story_project_destroy(&active_story_project); story_spatial_registry_destroy(&active_spatial_registry)
	...
	graph_import_story(
		&active_story_project,
	); campaign_playthrough.active_case = index; campaign_playthrough.results[index].started = true; campaign_playthrough.results[index].case_id = item.id; campaign_playthrough.results[index].case_content_version = item.case_content_version; _ = save_campaign_playthrough(); destination: Screen = case_starts_in_city(g) ? .Exterior : .Investigate; reset_case_state(g, begin_new_story_seed(), destination); return {true, "CASE STARTED"}
}

campaign_load_case_now :: proc(g: ^Game, index: int) -> Validation {
	work := Case_Load_Work {
		index = index,
	}; loaded := campaign_prepare_case(index, &work); if !loaded.ok do return loaded
...
campaign_launch_case :: proc(g: ^Game, index: int) -> Validation {
	...
	item := campaign_document.cases[index]
	campaign_case_load_reap(); case_load_work = Case_Load_Work {
		state = 1,
		index = index,
	}; case_load_work.worker = thread.create(
		campaign_case_load_worker,
	); if case_load_work.worker == nil do return {false, "could not start case loader"}; thread.start(case_load_work.worker)
	g.case_loading_active = true; g.case_loading_index = index; g.case_loading_title = item.title
	g.map_loading_active =
		true; g.map_loading_target = .Investigate; g.map_loading_progress = 0; g.map_loading_elapsed = 0; g.map_loading_stage = 0
	g.screen = .Loading
	return {true, "LOADING CASE"}
}

campaign_continue_selected :: proc(g: ^Game) -> Validation {if g.menu_return !=
	   .Title {continue_from_title(g); return{true, "CASE RESUMED"}}
	selected := campaign_playthrough.active_case
	if selected < 0 ||
	   selected >= len(campaign_document.cases) ||
	   !campaign_case_unlocked(&campaign_document, &campaign_playthrough, selected) {selected = -1
		for _, i in campaign_document.cases do if campaign_case_unlocked(&campaign_document, &campaign_playthrough, i) {selected = i; break}}
	return campaign_launch_case(g, selected)}

campaign_browser_viewport :: proc() -> Rect {return {120, 174, 960, 396}}
campaign_browser_content_height :: proc() -> f32 {return max(
		0,
		f32(campaign_browser.count) * 96 - 12,
	)}
campaign_browser_card_logical_rect :: proc(index: int) -> Rect {return{
		120,
		174 + f32(index) * 96,
		944,
		84,
	}}
campaign_browser_card_rect :: proc(index: int) -> Rect {box := campaign_browser_card_logical_rect(
		index,
	)
	box.y -= campaign_browser.scroll
	return box}
campaign_browser_card_id :: proc(index: int) -> ui.Gui_Id {return button_id(
		campaign_browser_card_logical_rect(index),
	)}
campaign_browser_card_button :: proc(g: ^Game, index: int) -> bool {box :=
		campaign_browser_card_rect(index)
	return ui.gui_button_at(
		&g.gui,
		campaign_browser_card_id(index),
		{box.x, box.y, box.w, box.h},
		"",
		true,
	)}
campaign_case_card_rect :: proc(slot: int) -> Rect {return {120, 174 + f32(slot) * 92, 960, 76}}
campaign_first_unlocked_on_page :: proc(page: int) -> int {first := page * 4; last := min(
		first + 4,
		len(campaign_document.cases),
	)
	for i in first ..< last do if campaign_case_unlocked(&campaign_document, &campaign_playthrough, i) do return i
	return -1}
campaign_focus_visible_selection :: proc(g: ^Game) {
	...
	if g.screen ==
	   .Campaign {for i in 0 ..< campaign_browser.count do if g.gui.focused == campaign_browser_card_id(i) {campaign_browser.selected = i; return}}
	if g.screen ==
...
update :: proc(g: ^Game) {
	...
	ui.gui_begin_frame(
		...
		ui.Input_State {
			...
			mouse_pos = {g.input.mouse_pos.x, g.input.mouse_pos.y},
			...
			pointer_enabled = g.active_device != .Gamepad,
			active_device = g.active_device == .Gamepad ? .Controller : .Mouse_Keyboard,
			nav_pressed_x = (g.input.right ? f32(1) : g.input.left ? f32(-1) : 0),
			nav_pressed_y = (g.input.down ? f32(1) : g.input.up ? f32(-1) : 0),
			accept_pressed = g.input.activate,
			back = g.input.back,
			text_input = g.input.text_input,
			text_input_len = g.input.text_input_len,
			clipboard_paste = g.input.clipboard_paste,
			clipboard_paste_len = g.input.clipboard_paste_len,
			key_shift = g.input.key_shift,
			key_ctrl = g.input.key_ctrl,
			key_super = g.input.key_super,
			key_enter = g.input.key_enter,
			key_escape = g.input.key_escape,
			key_backspace = g.input.key_backspace,
			key_delete = g.input.key_delete,
			key_home = g.input.key_home,
			key_end = g.input.key_end,
			key_left = g.input.key_left,
			key_right = g.input.key_right,
			key_a = g.input.key_a,
			key_x = g.input.key_x,
			key_v = g.input.key_v,
			key_c = g.input.key_c,
	...
	if menu_screen(g.screen) &&
	   (!g.focus_screen_initialized ||
			   g.focus_screen != g.screen ||
			   g.gui.focused ==
				   ui.GUI_ID_NONE) {g.gui.focused = button_id(menu_default_rect(g)); g.focus_screen = g.screen; g.focus_screen_initialized = true}
	...
	case .Campaign_Cases:
		first := campaign_case_page * 4; last := min(first + 4, len(campaign_document.cases))
		for i in first ..< last {if campaign_case_unlocked(&campaign_document, &campaign_playthrough, i) && button(g, campaign_case_card_rect(i - first)) do campaign_playthrough.active_case = i}
		pages := max((len(campaign_document.cases) + 3) / 4, 1)
		page_delta := g.input.shoulder_left ? -1 : g.input.shoulder_right ? 1 : 0
		if pages >
	...
	case .Options:
		...
		if button(
			g,
			{410, 245, 380, 48},
		) {g.aa_mode = Anti_Aliasing_Mode((int(g.aa_mode) + 1) % len(Anti_Aliasing_Mode)); g.aa_restart_required = true; persist_game_options(g)}
		if button(
			g,
			{410, 345, 380, 48},
		) {g.lighting_quality = Lighting_Quality((int(g.lighting_quality) + 1) % len(Lighting_Quality)); persist_game_options(g)}
		if button(
			g,
			{410, 445, 380, 48},
	...
	case .Pause:
		...
		if button(
			g,
			{410, 266, 380, 48},
		) {g.pause_feedback = pause_save_game(g) ? "GAME SAVED" : "SAVE FAILED"}
		if button(
			g,
			{410, 322, 380, 48},
	...
	case .Investigate:
		...
		if !player_package_mode &&
		   g.keys[.F10] &&
		   !g.build_key_latch &&
		   !editor_state.object_rotate_active {if editor_state.playtesting do _ = editor_end_playtest(g)
			else if g.editor_mode == .Build do authoring_app_request_build_exit(g)
			else {g.editor_mode = .Build; g.move_target_active = false}}; g.build_key_latch = g.keys[.F10]
		if g.editor_mode == .Graph {update_graph_mode(g); break}
		if g.editor_mode == .Build {
			if editor_state.feedback_frames > 0 do editor_state.feedback_frames -= 1
			else do editor_state.feedback = ""
			...
			if editor_viewport_contains(
				g.input.mouse_pos,
				g.build_tool,
			) {wx, wy, ok := editor_mouse_ground(g, g.input.mouse_pos); if ok {editor_state.cursor_world = {wx, wy}; editor_state.cursor_world_valid = true}}
			if g.build_tool == .Plant {
				...
				editor_state.placement_rotate_left_latch =
					left_down; editor_state.placement_rotate_right_latch = right_down
				wx, wy, ground_ok := editor_mouse_ground(
					g,
					g.input.mouse_pos,
				); editor_state.placement_active = ground_ok && editor_viewport_contains(g.input.mouse_pos, g.build_tool)
			...
			if g.build_tool == .Room &&
			   editor_state.room_draw_count >= 3 &&
			   g.input.activate {command := Level_Command {
					kind        = .Create_Room_Polygon,
					material    = editor_state.room_exterior ? "flagstone" : strings.to_lower(fmt.tprintf("%v", g.build_surface)),
					destination = editor_state.room_exterior ? "patio" : "",
					point_count = editor_state.room_draw_count,
				}; copy(
					command.points[:],
					editor_state.room_draw_points[:],
				); if level_commit_transaction(&level_document, command, editor_state.room_exterior ? "Create polygon patio" : "Create polygon room") do editor_state.room_draw_count = 0}
			...
			if g.build_tool == .Path &&
			   editor_state.path_draw_count >= 2 &&
			   g.input.activate {command := Level_Command {
					kind        = .Add_Path,
					c           = {f32(editor_state.path_kind), 0},
					value       = editor_state.path_width,
					point_count = editor_state.path_draw_count,
					material    = editor_state.path_kind == .Road ? "asphalt" : "gravel",
				}; copy(
					command.points[:],
					editor_state.path_draw_points[:],
				); if level_commit_transaction(&level_document, command, "Create terrain path") do editor_state.path_draw_count = 0}
			if g.build_tool == .Water &&
			   editor_state.water_draw_count >= 3 &&
			   g.input.activate {command := Level_Command {
					kind        = .Create_Water,
					value       = editor_state.water_elevation,
					point_count = editor_state.water_draw_count,
				}; copy(
					command.points[:],
					editor_state.water_draw_points[:],
				); if level_commit_transaction(&level_document, command, "Create pond") do editor_state.water_draw_count = 0}
			if button(g, editor_top_close_rect()) do authoring_app_request_build_exit(g)
			if (level_document.dirty || graph_document.dirty) &&
			   (button(g, editor_top_save_rect()) ||
					   g.input.save_document) {saved := authoring_app_save_all(); house_plan.validation = saved.message; if saved.ok do editor_show_feedback("ALL DOCUMENTS SAVED")
				else do editor_show_feedback("SAVE FAILED  ·  CHECK FILE PERMISSIONS", true)}
			if level_history.undo_count > 0 &&
			   (button(g, editor_top_undo_rect()) ||
					   g.input.undo_document) {if level_undo(&level_document) do editor_show_feedback("UNDO RESTORED PREVIOUS STATE")}
			if level_history.redo_count > 0 &&
			   (button(g, editor_top_redo_rect()) ||
					   g.input.redo_document) {if level_redo(&level_document) do editor_show_feedback("REDO REAPPLIED CHANGE")}
			...
			if editor_state.recovery_available &&
			   button(
				   g,
				   editor_top_recovery_rect(),
			...
			if g.build_tool !=
			   .Room {editor_state.room_draw_count = 0; editor_state.room_rectangle_active = false}
			if g.build_tool !=
			   .Foundation {editor_state.foundation_rectangle_active = false; editor_state.foundation_draw_count = 0}
			...
			if editor_state.selection_count == 1 &&
			   (editor_state.selection[0].kind == .Room ||
					   ...
					   editor_state.selection[0].kind ==
						   .Vertex) {room_index := level_room_index(&level_document, editor_state.selection[0].entity_id); if room_index >= 0 && button(g, editor_compact_numeric_rect(.Room_Level)) do editor_begin_numeric_edit(.Room_Level, level_document.rooms[room_index].platform_height)}
			if editor_state.selection_count == 1 &&
			   editor_state.selection[0].kind ==
				   .Foundation {index := level_foundation_index(&level_document, editor_state.selection[0].entity_id); if index >= 0 && button(g, editor_compact_numeric_rect(.Foundation_Measure)) {foundation := level_document.foundations[index]; editor_begin_numeric_edit(.Foundation_Measure, foundation.kind == .Raised ? foundation.elevation : foundation.depth)}}
			if editor_state.selection_count == 1 &&
			   editor_state.selection[0].kind ==
				   .Path {index := level_path_index(&level_document, editor_state.selection[0].entity_id); if index >= 0 && button(g, editor_compact_numeric_rect(.Path_Width)) do editor_begin_numeric_edit(.Path_Width, level_document.paths[index].width)}
			if editor_state.selection_count == 1 &&
			   editor_state.selection[0].kind ==
				   .Water {index := level_water_index(&level_document, editor_state.selection[0].entity_id); if index >= 0 && button(g, editor_compact_numeric_rect(.Water_Surface)) do editor_begin_numeric_edit(.Water_Surface, level_document.waters[index].elevation)}
			if editor_state.selection_count == 1 &&
			   editor_state.selection[0].kind ==
				   .Vertical_Link {index := level_vertical_link_index(&level_document, editor_state.selection[0].entity_id); if index >= 0 && button(g, editor_compact_numeric_rect(.Link_Width)) do editor_begin_numeric_edit(.Link_Width, level_document.vertical_links[index].width)}
			if editor_state.selection_count == 1 &&
			   editor_state.selection[0].kind ==
				   .Roof {index := level_roof_index(&level_document, editor_state.selection[0].entity_id); if index >= 0 {roof := level_document.roofs[index]; if button(g, editor_roof_numeric_rect(.Roof_Pitch)) do editor_begin_numeric_edit(.Roof_Pitch, roof.pitch); if button(g, editor_roof_numeric_rect(.Roof_Overhang)) do editor_begin_numeric_edit(.Roof_Overhang, roof.overhang); if button(g, editor_roof_numeric_rect(.Roof_Ridge)) do editor_begin_numeric_edit(.Roof_Ridge, roof.ridge_angle)}}
			if editor_state.selection_count == 1 &&
			   editor_state.selection[0].kind ==
			...
			if editor_state.selection_count == 1 &&
			   editor_state.selection[0].kind ==
				   .Marker {index := level_marker_index(&level_document, editor_state.selection[0].entity_id); if index >= 0 && button(g, editor_marker_name_rect()) do editor_begin_marker_name_edit(level_document.markers[index])}
			if editor_state.selection_count == 1 &&
			   editor_state.selection[0].kind == .Marker &&
			   button(
				   g,
				   editor_marker_duplicate_rect(),
			   ) {step := editor_state.snap_mode == .Fine ? level_document.fine_snap : level_document.default_snap; if editor_copy_selection() do _ = editor_paste_selection({step, step}, "DUPLICATED MARKER")}
			...
			if editor_state.selection_count == 1 &&
			   ...
			   button(
				   g,
				   editor_marker_open_graph_rect(),
			   ) {index := level_marker_index(&level_document, editor_state.selection[0].entity_id); if index >= 0 do _ = editor_open_marker_in_graph(g, level_document.markers[index])}
			if editor_state.selection_count == 1 &&
			   editor_state.selection[0].kind == .Light &&
			   button(
				   g,
				   editor_light_duplicate_rect(),
			   ) {step := editor_state.snap_mode == .Fine ? level_document.fine_snap : level_document.default_snap; if editor_copy_selection() do _ = editor_paste_selection({step, step}, "DUPLICATED LIGHT")}
			...
			if g.build_tool ==
			   .Path {if button(g, editor_link_kind_rect()) do editor_state.path_kind = editor_state.path_kind == .Road ? .Footpath : .Road; if button(g, editor_link_width_rect(0)) do editor_state.path_width = max(.2, editor_state.path_width - .2); if button(g, editor_link_width_rect(1)) do editor_state.path_width = min(8, editor_state.path_width + .2)}
			if g.build_tool ==
			   .Water {if button(g, editor_water_height_rect(0)) do editor_state.water_elevation -= .25; if button(g, editor_water_height_rect(1)) do editor_state.water_elevation += .25}
			if g.build_tool ==
			   .Foundation {for kind in Level_Foundation_Kind {index := int(kind); if button(g, editor_foundation_kind_rect(index)) {editor_state.foundation_kind = kind; if kind == .Raised do editor_state.foundation_elevation = max(editor_state.foundation_elevation, .5)
						else do editor_state.foundation_elevation = 0}}; if button(g, editor_foundation_measure_rect(0)) {if editor_state.foundation_kind == .Raised do editor_state.foundation_elevation = max(.25, editor_state.foundation_elevation - .25)
					else if editor_state.foundation_kind == .Basement do editor_state.foundation_depth = max(1.8, editor_state.foundation_depth - .25)
					else do editor_state.foundation_depth = max(.1, editor_state.foundation_depth - .05)}; if button(g, editor_foundation_measure_rect(1)) {if editor_state.foundation_kind == .Raised do editor_state.foundation_elevation = min(3, editor_state.foundation_elevation + .25)
					else if editor_state.foundation_kind == .Basement do editor_state.foundation_depth = min(6, editor_state.foundation_depth + .25)
					else do editor_state.foundation_depth = min(1, editor_state.foundation_depth + .05)}; if button(g, editor_foundation_mode_rect(0)) {editor_state.foundation_mode = .Rectangle; editor_state.foundation_draw_count = 0}; if button(g, editor_foundation_mode_rect(1)) {editor_state.foundation_mode = .Polygon; editor_state.foundation_rectangle_active = false}; if editor_state.foundation_kind == .Basement do editor_state.foundation_depth = max(editor_state.foundation_depth, 2.5); if editor_state.foundation_kind == .Raised do editor_state.foundation_elevation = max(editor_state.foundation_elevation, .25)}
			if g.build_tool ==
			   .Marker {for kind in Level_Marker_Kind {index := int(kind); if button(g, editor_marker_kind_rect(index)) do editor_state.marker_kind = kind}; if button(g, editor_marker_parameter_rect(0)) do editor_state.marker_radius = max(.1, editor_state.marker_radius - .1); if button(g, editor_marker_parameter_rect(1)) do editor_state.marker_radius = min(12, editor_state.marker_radius + .1); if button(g, editor_marker_parameter_rect(2)) do editor_state.marker_facing -= 15; if button(g, editor_marker_parameter_rect(3)) do editor_state.marker_facing += 15}
			if g.build_tool ==
			...
			if g.build_tool == .Terrain {
				for mode in Terrain_Brush_Mode {index := int(mode)
					if button(g, editor_terrain_mode_rect(index)) do editor_state.terrain_mode = mode}
			...
			if g.build_tool != .Terrain &&
			   editor_state.selection_count > 0 &&
			   editor_state.selection[0].kind ==
				   .Foundation {shell, ok := editor_selection_action_rect(g, 0); remove, _ := editor_selection_action_rect(g, 1); if ok && button(g, shell) do _ = editor_create_room_from_foundation(g, &level_document, editor_state.selection[0].entity_id); if editor_state.selection_count > 0 && editor_state.selection[0].kind == .Foundation && button(g, remove) && level_delete_selection(&level_document, editor_state.selection[0]) do editor_state.selection_count = 0}
			if g.build_tool != .Terrain &&
			   editor_state.selection_count > 0 &&
			   editor_state.selection[0].kind ==
				   .Path {remove, ok := editor_selection_action_rect(g, 0); if ok && button(g, remove) && level_delete_selection(&level_document, editor_state.selection[0]) do editor_state.selection_count = 0}
			if g.build_tool != .Terrain &&
			   editor_state.selection_count > 0 &&
			   editor_state.selection[0].kind ==
				   .Water {remove, ok := editor_selection_action_rect(g, 0); if ok && button(g, remove) && level_delete_selection(&level_document, editor_state.selection[0]) do editor_state.selection_count = 0}
			if g.build_tool != .Terrain &&
			   editor_state.selection_count > 0 &&
			   editor_state.selection[0].kind ==
				   .Vertical_Link {remove, ok := editor_selection_action_rect(g, 0); if ok && button(g, remove) && level_delete_selection(&level_document, editor_state.selection[0]) do editor_state.selection_count = 0}
			...
			if editor_state.selection_count == 2 &&
			   editor_state.selection[0].kind == .Room &&
			   editor_state.selection[1].kind ==
				   .Room {merge_command := level_merge_room_command(editor_state.selection[0].entity_id, editor_state.selection[1].entity_id); if level_preview_transaction(&level_document, merge_command).state != .Blocked && button(g, {778, 608, 36, 28}) && level_commit_transaction(&level_document, merge_command, "Merge rooms") {editor_state.selection[0] = {.Room, merge_command.entity_id, -1}; editor_state.selection_count = 1}}
			if g.build_tool != .Terrain &&
			   editor_state.selection_count > 0 &&
			   editor_state.selection[0].kind ==
				   .Marker {remove, ok := editor_selection_action_rect(g, 0); if ok && button(g, remove) && level_delete_selection(&level_document, editor_state.selection[0]) do editor_state.selection_count = 0}
			if editor_state.selection_count > 0 &&
			   editor_state.selection[0].kind ==
				   .Marker {index := level_marker_index(&level_document, editor_state.selection[0].entity_id); if index >= 0 {marker := level_document.markers[index]; if button(g, editor_inspector_step_rect(178, -1)) {if marker.reference != "" || marker.destination != "" do editor_show_feedback("CLEAR THE MARKER BINDING BEFORE CHANGING TYPE", true)
						else {marker.kind = Level_Marker_Kind((int(marker.kind) - 1 + len(Level_Marker_Kind)) % len(Level_Marker_Kind)); _ = level_commit_transaction(&level_document, marker_edit_command(marker), "Change marker type")}}; if button(g, editor_inspector_step_rect(178, 1)) {if marker.reference != "" || marker.destination != "" do editor_show_feedback("CLEAR THE MARKER BINDING BEFORE CHANGING TYPE", true)
						else {marker.kind = Level_Marker_Kind((int(marker.kind) + 1) % len(Level_Marker_Kind)); _ = level_commit_transaction(&level_document, marker_edit_command(marker), "Change marker type")}}; if level_marker_uses_binding(marker.kind) && button(g, editor_inspector_step_rect(248, -1)) {binding := marker_binding_next(g, marker, -1); if marker.kind == .Transition do marker.destination = binding
						else do marker.reference = binding; _ = level_commit_transaction(&level_document, marker_edit_command(marker), "Bind marker")}; if level_marker_uses_binding(marker.kind) && button(g, editor_inspector_clear_rect(248)) {marker.reference = ""; marker.destination = ""; _ = level_commit_transaction(&level_document, marker_edit_command(marker), "Clear marker binding")}; if level_marker_uses_binding(marker.kind) && button(g, editor_inspector_step_rect(248, 1)) {binding := marker_binding_next(g, marker, 1); if marker.kind == .Transition do marker.destination = binding
						else do marker.reference = binding; _ = level_commit_transaction(&level_document, marker_edit_command(marker), "Bind marker")}; if marker.radius > .1 && button(g, editor_inspector_step_rect(318, -1)) {marker.radius = max(.1, marker.radius - .1); _ = level_commit_transaction(&level_document, marker_edit_command(marker), "Reduce marker radius")}; if marker.radius < 12 && button(g, editor_inspector_step_rect(318, 1)) {marker.radius = min(12, marker.radius + .1); _ = level_commit_transaction(&level_document, marker_edit_command(marker), "Increase marker radius")}; if button(g, editor_inspector_step_rect(354, -1)) {marker.facing -= 15; _ = level_commit_transaction(&level_document, marker_edit_command(marker), "Rotate marker")}; if button(g, editor_inspector_step_rect(354, 1)) {marker.facing += 15; _ = level_commit_transaction(&level_document, marker_edit_command(marker), "Rotate marker")}; if marker.kind == .Camera {if marker.camera_height > .1 && button(g, editor_inspector_step_rect(390, -1)) {marker.camera_height = max(.1, marker.camera_height - .1); _ = level_commit_transaction(&level_document, marker_edit_command(marker), "Lower camera marker")}; if button(g, editor_inspector_step_rect(390, 1)) {marker.camera_height += .1; _ = level_commit_transaction(&level_document, marker_edit_command(marker), "Raise camera marker")}}}}
			if editor_state.selection_count > 0 &&
			   editor_state.selection[0].kind ==
			...
			if g.input.mouse_pressed &&
			   ...
			   editor_viewport_contains(
				   g.input.mouse_pos,
				   g.build_tool,
			   ) {wx, wy, ok := editor_mouse_ground(g, g.input.mouse_pos); if ok {point := level_snap_point(&level_document, {wx, wy}, true); if !editor_state.link_anchor_active {editor_state.link_anchor = point; editor_state.link_finish = point; editor_state.link_anchor_active = true} else {command := Level_Command {
							kind  = .Create_Vertical_Link,
							a     = editor_state.link_anchor,
							b     = point,
							c     = {f32(editor_state.link_kind), 0},
							value = editor_state.link_width,
						}; if level_preview_transaction(&level_document, command).state != .Blocked && level_commit_transaction(&level_document, command, "Create vertical link") do editor_state.link_anchor_active = false}}}
			if g.input.mouse_pressed &&
			   g.build_tool == .Path &&
			   editor_viewport_contains(
				   g.input.mouse_pos,
				   g.build_tool,
			   ) {wx, wy, ok := editor_mouse_ground(g, g.input.mouse_pos); if ok && editor_state.path_draw_count < len(editor_state.path_draw_points) {editor_state.path_draw_points[editor_state.path_draw_count] = level_snap_point(&level_document, {wx, wy}, true); editor_state.path_draw_count += 1}}
			if g.input.mouse_pressed &&
			   g.build_tool == .Water &&
			   editor_viewport_contains(
				   g.input.mouse_pos,
				   g.build_tool,
			   ) {wx, wy, ok := editor_mouse_ground(g, g.input.mouse_pos); if ok {point := level_snap_point(&level_document, {wx, wy}, true); close := false; if editor_state.water_draw_count >= 3 {first := editor_state.water_draw_points[0]; dx, dy := point.x - first.x, point.y - first.y; close = dx * dx + dy * dy <= .35 * .35}; if close {command := Level_Command {
							kind        = .Create_Water,
							value       = editor_state.water_elevation,
							point_count = editor_state.water_draw_count,
						}; copy(
							command.points[:],
							editor_state.water_draw_points[:],
						); if level_commit_transaction(&level_document, command, "Create pond") do editor_state.water_draw_count = 0} else if editor_state.water_draw_count < len(editor_state.water_draw_points) {editor_state.water_draw_points[editor_state.water_draw_count] = point; editor_state.water_draw_count += 1}}}
			if g.build_tool == .Room && editor_state.room_mode == .Rectangle {
				if g.input.mouse_pressed &&
				   editor_viewport_contains(
					   g.input.mouse_pos,
					   g.build_tool,
				   ) {wx, wy, ok := editor_mouse_ground(g, g.input.mouse_pos); if ok {point := level_snap_point(&level_document, {wx, wy}, true); editor_state.room_rectangle_start = point; editor_state.room_rectangle_current = point; editor_state.room_rectangle_active = true; editor_state.room_rectangle_preview = {.Blocked, "DRAG TO SIZE ROOM", point, point}}}
			...
			if g.build_tool == .Foundation && editor_state.foundation_mode == .Rectangle {
				...
				if editor_state.foundation_rectangle_active &&
				   (g.input.mouse_down ||
						   g.input.mouse_released) {wx, wy, ok := editor_mouse_ground(g, g.input.mouse_pos); if ok {editor_state.foundation_rectangle_current = level_snap_point(&level_document, {wx, wy}, true); a, b := editor_state.foundation_rectangle_start, editor_state.foundation_rectangle_current; command := Level_Command {
							kind        = .Create_Foundation,
							value       = editor_state.foundation_elevation,
							c           = {
								f32(editor_state.foundation_kind),
								editor_state.foundation_depth,
							},
							point_count = 4,
						}; command.points[0] = {
							min(a.x, b.x),
							min(a.y, b.y),
						}; command.points[1] = {max(a.x, b.x), min(a.y, b.y)}; command.points[2] = {max(a.x, b.x), max(a.y, b.y)}; command.points[3] = {min(a.x, b.x), max(a.y, b.y)}; editor_state.foundation_rectangle_preview = level_preview_transaction(&level_document, command)}}
				if editor_state.foundation_rectangle_active &&
				   g.input.mouse_released {if editor_state.foundation_rectangle_preview.state != .Blocked {a, b := editor_state.foundation_rectangle_start, editor_state.foundation_rectangle_current; command := Level_Command {
							kind        = .Create_Foundation,
							value       = editor_state.foundation_elevation,
							c           = {
								f32(editor_state.foundation_kind),
							...
							min(a.x, b.x),
							min(a.y, b.y),
						}; command.points[1] = {max(a.x, b.x), min(a.y, b.y)}; command.points[2] = {max(a.x, b.x), max(a.y, b.y)}; command.points[3] = {min(a.x, b.x), max(a.y, b.y)}; foundation_count := len(level_document.foundations); if level_commit_transaction(&level_document, command, fmt.tprintf("Create %s foundation", level_foundation_kind_name(editor_state.foundation_kind))) && !g.keys[.LSHIFT] && !g.keys[.RSHIFT] && len(level_document.foundations) > foundation_count do _ = editor_advance_from_foundation(g, &level_document, len(level_document.foundations) - 1)}; editor_state.foundation_rectangle_active = false}
			}
			if g.input.mouse_pressed &&
			   !editor_state.drag_active &&
			   !editor_state.box_select_active &&
			   !editor_state.terrain_stroke_active &&
			   editor_viewport_contains(g.input.mouse_pos, g.build_tool) {
				wx, wy, ok := editor_mouse_ground(g, g.input.mouse_pos)
				if ok {
					point := Vec2 {
						wx,
						wy,
					}; picked, handle_ok := level_pick_control_point(&level_document, point, editor_state.selection[0]); if !handle_ok do picked, handle_ok = level_pick_room_handle(&level_document, point, editor_state.selection[0]); if !handle_ok do picked = level_pick(&level_document, point)
					if g.build_tool == .Select do picked = editor_view_pick_selection(&level_document, editor_state.view, picked)
					if g.keys[.G] &&
					   picked.kind ==
						   .Room {_ = level_commit_transaction(&level_document, Level_Command{kind = .Paint_Room, entity_id = picked.entity_id, material = "__grounds__"}, "Make room exterior")} else {
						#partial switch g.build_tool {
						case .Select:
							editor_select_pointer(g, picked, point)
						case .Terrain:
							editor_state.terrain_stroke_active = true
							editor_state.terrain_stroke_start = point
							editor_state.terrain_stroke_current = point
							editor_state.terrain_sample = level_terrain_height(
								&level_document,
								point,
							)
							editor_state.selection_count = 0
						case .Marker:
							command := Level_Command {
								...
								b           = {
									...
									editor_state.marker_facing,
								},
								c           = {
									editor_state.marker_camera_height,
									f32(editor_state.marker_kind),
								},
								material    = editor_state.marker_reference,
								destination = editor_state.marker_destination,
								value       = f32(level_document.active_story),
							}; _ = level_commit_transaction(
								&level_document,
								command,
								fmt.tprintf(
									"Place %s marker",
									level_marker_kind_name(editor_state.marker_kind),
								),
							)
						case .Light:
							command := Level_Command {
								kind  = .Add_Light,
								a     = point,
								b     = {editor_state.light_range, editor_state.light_intensity},
								c     = {
									editor_state.light_elevation,
									f32(editor_state.light_kind),
								},
								value = f32(level_document.active_story),
								color = editor_state.light_color,
							}; command.points[0] = {
								editor_state.light_cone_angle,
								editor_state.light_facing,
							}; if level_commit_transaction(&level_document, command, fmt.tprintf("Place %s light", level_light_kind_name(editor_state.light_kind))) {editor_state.selection[0] = {.Light, level_document.lights[len(level_document.lights) - 1].id, -1}; editor_state.selection_count = 1}
						case .Paint, .Wall_Paint:
							if picked.kind ==
							   .Room {room_index := level_room_index(&level_document, picked.entity_id); if editor_state.paint_eyedropper && room_index >= 0 {material := editor_room_sample_material(level_document.rooms[room_index], editor_state.paint_target); if material != "" {editor_state.catalog_id = material; catalog_record_recent(&editor_state, material)}; editor_state.paint_eyedropper = false} else {paint_kind := editor_paint_command_kind(editor_state.paint_target, g.keys[.LSHIFT] || g.keys[.RSHIFT]); material := editor_state.catalog_id; if material == "" do material = "dining"; label := paint_kind == .Paint_Floor ? "Paint floor" : paint_kind == .Paint_Walls ? "Paint walls" : "Paint whole room"; _ = level_commit_transaction(&level_document, Level_Command{kind = paint_kind, entity_id = picked.entity_id, material = material}, label)}}
						case .Plant:
							if editor_state.placement_preview.state != .Blocked do _ = level_commit_transaction(&level_document, Level_Command{kind = .Place_Object, a = editor_state.placement_position, c = {editor_state.placement_elevation, 0}, value = editor_state.placement_rotation, material = editor_state.catalog_id == "" ? "plant" : editor_state.catalog_id, destination = editor_state.placement_support_id}, editor_state.placement_support_id != "" ? "Place object on furniture" : "Place object")
						case .Door, .Window:
							if editor_state.opening_active && editor_state.opening_preview.state != .Blocked do _ = level_commit_transaction(&level_document, editor_state.opening_command, g.build_tool == .Window ? "Place window" : "Place door")
						case .Wall:
							if g.build_has_anchor {command, split := level_wall_command(&level_document, g.build_anchor, point); if level_preview_transaction(&level_document, command).state != .Blocked && level_commit_transaction(&level_document, command, split ? "Split room with wall" : "Draw freestanding wall") {g.build_has_anchor = false; editor_state.wall_preview_active = false; if split {editor_state.selection[0] = {.Room, command.entity_id, -1}; editor_state.selection_count = 1}}} else {g.build_anchor = level_snap_point(&level_document, point, true); g.build_has_anchor = true; editor_state.wall_preview_point = g.build_anchor; editor_state.wall_preview_active = true}
						case .Room:
							if editor_state.room_mode ==
							   .Polygon {snapped := level_snap_point(&level_document, point, true); close := false; if editor_state.room_draw_count >= 3 {first := editor_state.room_draw_points[0]; dx, dy := snapped.x - first.x, snapped.y - first.y; close = dx * dx + dy * dy <= .35 * .35}; if close {command := Level_Command {
										kind        = .Create_Room_Polygon,
										material    = editor_state.room_exterior ? "flagstone" : strings.to_lower(fmt.tprintf("%v", g.build_surface)),
										destination = editor_state.room_exterior ? "patio" : "",
										point_count = editor_state.room_draw_count,
	...
	case .Check:
		...
		roll_settled :=
			g.check_done && g.animation_time - g.check_roll_started >= CHECK_REVEAL_DURATION
		if !g.check_done &&
		   button(
			   g,
			   {430, 510, 340, 58},
		   ) {g.check_result = resolve_clue_check(g, g.pending_clue); g.check_roll_started = g.animation_time; g.check_result_cue_played = false; play_check_dice_sound(g); g.check_done = true}
		if roll_settled &&
		   g.check_done &&
		   button(
			   g,
	...
	case .Notebook:
		...
		if g.notebook_tab !=
		   old_tab {g.notebook_scroll = 0; g.notebook_scroll_target = 0; g.gui.focused = button_id({20 + f32(g.notebook_tab) * 190, 95, 176, 46})}
		scroll_delta := -g.input.mouse_wheel * 72 + g.pad_right_y * 12
		if g.input.up do scroll_delta -= 56
		if g.input.down do scroll_delta += 56
		g.notebook_scroll_target = clamp(
			g.notebook_scroll_target + scroll_delta,
			0,
			g.notebook_scroll_max,
		)
	...
	case .Game_Over:
		if button(
			...
			reset_case_state(g, g.run_seed, destination)}
		if button(g, {410, 478, 380, 52}) {reset_case_state(g, begin_new_story_seed(), .Title)}
		if button(g, {410, 546, 380, 52}) {g.running = false}
	case .Reveal_Prep:
		...
		if button(
			g,
			{400, 620, 400, 58},
		) {if mystery_game_accusation(g) == "" {g.result = evaluate_questions(g)
				if !enter_case_ending(
					g,
