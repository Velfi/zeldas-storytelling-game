...
Project_Asset_Missing_Record :: struct {
	asset_id, expected_path: string,
}
Project_Asset_Relink_Candidate :: struct {
	asset_id, candidate_path, sha256: string,
	exact_hash:                       bool,
}
...
Project_Asset_Stage_Item :: struct {
	asset_id, source_path, package_path, sha256: string,
	byte_size:                                   u64,
}
...
project_asset_registry_serialize :: proc(registry: ^Project_Asset_Registry) -> string {
	...
	for asset in registry.assets {
		text = fmt.tprintf(
			...
			text,
			project_asset_io_escape(asset.id),
			int(asset.kind),
			project_asset_io_escape(asset.project_path),
			project_asset_io_escape(asset.source_path),
			asset.sha256,
			int(asset.source_mode),
			int(asset.embed_policy),
			project_asset_io_escape(asset.provenance.source_uri),
			project_asset_io_escape(asset.provenance.source_name),
			project_asset_io_escape(asset.provenance.creator),
			project_asset_io_escape(asset.provenance.attribution),
			project_asset_io_escape(asset.provenance.license_id),
			project_asset_io_escape(asset.provenance.license_text),
			asset.provenance.redistribution_permitted,
			project_asset_io_escape(asset.technical.format),
			asset.technical.byte_size,
			asset.technical.image.width,
			asset.technical.image.height,
			project_asset_io_escape(asset.technical.image.color_space),
			asset.technical.image.has_alpha,
			asset.technical.audio.duration_seconds,
			asset.technical.audio.channels,
			asset.technical.audio.sample_rate,
			asset.technical.model.bounds_min[0],
			asset.technical.model.bounds_min[1],
			asset.technical.model.bounds_min[2],
			asset.technical.model.bounds_max[0],
			asset.technical.model.bounds_max[1],
			asset.technical.model.bounds_max[2],
			asset.technical.model.meters_per_unit,
			project_asset_io_escape(asset.technical.model.up_axis),
			project_asset_io_escape(asset.technical.model.forward_axis),
...
project_asset_registry_save :: proc(
	path: string,
	...
	if os.rename(temporary, path) !=
	   nil {_ = os.remove(temporary); return {false, "could not replace asset registry"}}
...
project_asset_registry_load :: proc(path: string, out: ^Project_Asset_Registry) -> Validation {
	...
	); if error != nil do return {false, "invalid asset registry path"}
	...
	stored_revision := u64(
		max(toml_case_int(top, "revision"), 0),
	); next := Project_Asset_Registry{}
	for table in toml_tables(top, "assets") {
		asset := Project_Asset_Record {
			id           = toml_case_string(table, "id"),
			kind         = Project_Asset_Kind(
				clamp(toml_case_int(table, "kind"), 0, int(Project_Asset_Kind.Thumbnail)),
			),
			project_path = toml_case_string(table, "project_path"),
			source_path  = toml_case_string(table, "source_path"),
			sha256       = toml_case_string(table, "sha256"),
			source_mode  = Project_Asset_Source_Mode(
				clamp(toml_case_int(table, "source_mode"), 0, 1),
			),
			embed_policy = Project_Asset_Embed_Policy(
				clamp(toml_case_int(table, "embed_policy"), 0, 2),
			),
		}
		asset.provenance = {
			source_uri               = toml_case_string(table, "source_uri"),
			source_name              = toml_case_string(table, "source_name"),
			creator                  = toml_case_string(table, "creator"),
			attribution              = toml_case_string(table, "attribution"),
			license_id               = toml_case_string(table, "license_id"),
			license_text             = toml_case_string(table, "license_text"),
			redistribution_permitted = toml_case_bool(table, "redistribution_permitted"),
		}
		asset.technical.format = toml_case_string(
			table,
			"format",
		); asset.technical.byte_size = u64(max(toml_case_int(table, "byte_size"), 0)); asset.technical.image = {
			width       = toml_case_int(table, "image_width"),
			height      = toml_case_int(table, "image_height"),
			color_space = toml_case_string(table, "color_space"),
			has_alpha   = toml_case_bool(table, "has_alpha"),
		}; asset.technical.audio = {
			duration_seconds = f64(toml_case_float(table, "audio_duration")),
			channels         = toml_case_int(table, "audio_channels"),
			sample_rate      = toml_case_int(table, "audio_sample_rate"),
		}
		asset.technical.model.bounds_min = {
			toml_case_float(table, "model_min_x"),
			toml_case_float(table, "model_min_y"),
			toml_case_float(table, "model_min_z"),
		}; asset.technical.model.bounds_max = {toml_case_float(table, "model_max_x"), toml_case_float(table, "model_max_y"), toml_case_float(table, "model_max_z")}
		asset.technical.model.meters_per_unit = toml_case_float(
			table,
			"meters_per_unit",
		); asset.technical.model.up_axis = toml_case_string(table, "up_axis"); asset.technical.model.forward_axis = toml_case_string(table, "forward_axis"); asset.technical.model.mesh_count = toml_case_int(table, "mesh_count"); asset.technical.model.material_count = toml_case_int(table, "material_count"); asset.technical.animation = {
			clip_count       = toml_case_int(table, "clip_count"),
			duration_seconds = f64(toml_case_float(table, "animation_duration")),
		}
		if valid := project_asset_registry_add(&next, asset);
		   !valid.ok {project_asset_registry_destroy(&next); return valid}
	}
	for table in toml_tables(
		top,
		"usages",
	) {usage := Project_Asset_Usage{toml_case_string(table, "asset_id"), toml_case_string(table, "document"), toml_case_string(table, "entity_id"), toml_case_string(table, "field_path")}; if !project_asset_registry_register_usage(&next, usage) {project_asset_registry_destroy(&next); return {false, "asset registry contains an invalid usage"}}}
	for table in toml_tables(
		top,
		"dependencies",
	) {dependency := Project_Asset_Dependency{toml_case_string(table, "asset_id"), toml_case_string(table, "depends_on_id"), toml_case_string(table, "purpose")}; if !project_asset_registry_register_dependency(&next, dependency) {project_asset_registry_destroy(&next); return {false, "asset registry contains an invalid dependency"}}}
	for table in toml_tables(
		top,
		"external_catalog_usages",
	) {usage := Project_External_Catalog_Usage {
			reference  = {
				toml_case_string(table, "namespace"),
				toml_case_string(table, "catalog_id"),
				toml_case_string(table, "version"),
			},
			document   = toml_case_string(table, "document"),
			entity_id  = toml_case_string(table, "entity_id"),
			field_path = toml_case_string(table, "field_path"),
		}; if valid := project_asset_registry_register_external_catalog_usage(&next, usage);
		   !valid.ok {project_asset_registry_destroy(&next); return valid}}
	next.revision =
		stored_revision; project_asset_registry_destroy(out); out^ = next; return {true, "ASSET REGISTRY LOADED"}
}
...
project_asset_registry_load_project :: proc(
	project: ^Authoring_Project,
	out: ^Project_Asset_Registry,
) -> Validation {
...
project_asset_registry_save_project :: proc(
	project: ^Authoring_Project,
	registry: ^Project_Asset_Registry,
) -> Validation {
...
project_asset_sha256_file :: proc(path: string) -> (string, Validation) {
	data, error := os.read_entire_file_from_path(
		path,
		context.temp_allocator,
	); if error != nil do return "", {false, "could not read asset for hashing"}
	ctx: sha2.Context_256; digest: [sha2.DIGEST_SIZE_256]byte; sha2.init_256(&ctx); sha2.update(&ctx, data); sha2.final(&ctx, digest[:])
	encoded, allocation_error := hex.encode(
		digest[:],
	); if allocation_error != nil do return "", {false, "could not encode asset hash"}
...
project_asset_stable_id :: proc(source_path: string, registry: ^Project_Asset_Registry) -> string {
	...
	for rune in base {if rune >= 'a' && rune <= 'z' || rune >= '0' && rune <= '9' || rune == '_' || rune == '-' do id = fmt.tprintf("%s%c", id, rune)
		else if id != "" && !strings.has_suffix(id, "-") do id = fmt.tprintf("%s-", id)}
...
project_asset_path_within_root :: proc(root, target: string) -> bool {
	...
	clean_root, clean_root_error := filepath.clean(
		root,
		context.allocator,
	); if clean_root_error != nil do return false; defer delete(clean_root)
	clean_target, clean_target_error := filepath.clean(
		target,
		context.allocator,
	); if clean_target_error != nil do return false; defer delete(clean_target)
...
project_asset_u32_be :: proc(data: []byte, at: int) -> u32 {return(
		u32(data[at]) << 24 |
		u32(data[at + 1]) << 16 |
		u32(data[at + 2]) << 8 |
		u32(data[at + 3]) \
	)}
project_asset_u32_le :: proc(data: []byte, at: int) -> u32 {return(
		u32(data[at]) |
		u32(data[at + 1]) << 8 |
		u32(data[at + 2]) << 16 |
		u32(data[at + 3]) << 24 \
	)}
project_asset_u16_le :: proc(data: []byte, at: int) -> u16 {return(
		u16(data[at]) |
		u16(data[at + 1]) << 8 \
	)}
project_asset_u64_le :: proc(data: []byte, at: int) -> u64 {result: u64; for i in 0 ..< 8 do result |= u64(data[at + i]) << u64(i * 8)
	return result}

project_asset_inspect_file :: proc(
	path: string,
...
project_asset_import :: proc(
	...
	hash, hashed := project_asset_sha256_file(
		request.source_path,
	); if !hashed.ok do return hashed; if project_asset_hash_index(registry, hash) >= 0 do return {false, "asset content is already registered"}
	id :=
		request.requested_id; if id == "" do id = project_asset_stable_id(request.source_path, registry); if !project_asset_id_valid(id) do return {false, "requested asset ID is invalid"}
	record := Project_Asset_Record {
		id           = id,
		kind         = request.kind,
	...
	}; inspected := project_asset_inspect_file(
		request.source_path,
		&record.technical,
	); if !inspected.ok do return inspected
	if request.mode ==
...
project_asset_record_path :: proc(root: string, asset: Project_Asset_Record) -> string {path :=
		asset.source_path
	if asset.project_path != "" {joined, error := filepath.join(
			[]string{root, asset.project_path},
			context.allocator,
		)
		if error == nil do path = joined}
	return path}

project_asset_plan_relink :: proc(
	registry: ^Project_Asset_Registry,
	project_root, search_root: string,
) -> Project_Asset_Relink_Plan {
	...
	for info in os.walker_walk(
		&walker,
	) {if info.type == .Regular {hash, valid := project_asset_sha256_file(info.fullpath); if !valid.ok do continue; for missing in result.missing {index := project_asset_index(registry, missing.asset_id); if index >= 0 && registry.assets[index].sha256 == hash do append(&result.candidates, Project_Asset_Relink_Candidate{missing.asset_id, strings.clone(info.fullpath), hash, true})}}}
	return result
}

project_asset_relink_plan_destroy :: proc(plan: ^Project_Asset_Relink_Plan) {for &item in plan.candidates do delete(item.candidate_path)
	delete(plan.missing)
	delete(plan.candidates)
	plan^ = {}}

project_asset_preview_replacement :: proc(
	registry: ^Project_Asset_Registry,
	id, candidate_path: string,
) -> Project_Asset_Replacement_Preview {
	result := Project_Asset_Replacement_Preview {
		asset_id       = strings.clone(id),
	...
	if !os.is_file(
		candidate_path,
	) {result.message = "replacement file does not exist"; return result}
	...
	if inspected := project_asset_inspect_file(candidate_path, &replacement.technical);
	   !inspected.ok {result.message = inspected.message; return result}
	if valid := project_asset_validate_record(replacement);
	   !valid.ok {result.message = valid.message; return result}
	duplicate := project_asset_hash_index(registry, hash)
	if duplicate >= 0 &&
	   duplicate != index {result.message = "replacement duplicates another asset"; return result}
	...
	result.message = fmt.tprintf(
		"READY · %d REFERENCES AND %d DEPENDENTS PRESERVED",
		len(result.change.usages),
		len(result.change.dependents),
	)
	return result
}
...
project_asset_commit_replacement :: proc(
	registry: ^Project_Asset_Registry,
	project_root: string,
	preview: ^Project_Asset_Replacement_Preview,
) -> Validation {
	...
	if replacement.source_mode == .Copy {
		...
		parent, _ := filepath.split(
			destination,
		); if !os.is_dir(parent) && os.make_directory_all(parent) != nil do return {false, "could not create replacement directory"}
		...
		if !checked.ok ||
		   staged_hash !=
			   hash {_ = os.remove(temporary); return {false, "staged replacement failed integrity verification"}}
		if os.rename(temporary, destination) !=
		   nil {_ = os.remove(temporary); return {false, "could not atomically install replacement"}}
...
project_asset_apply_relink :: proc(
	registry: ^Project_Asset_Registry,
	project_root: string,
	candidate: Project_Asset_Relink_Candidate,
) -> Validation {
	index := project_asset_index(
		registry,
		candidate.asset_id,
	); if index < 0 || !candidate.exact_hash do return {false, "relink requires an exact hash match"}; hash, valid := project_asset_sha256_file(candidate.candidate_path); if !valid.ok || hash != registry.assets[index].sha256 do return {false, "relink candidate content has changed"}; asset := &registry.assets[index]
	if asset.source_mode == .Copy {
		if project_asset_path_within_root(project_root, candidate.candidate_path) {
			relative, error := filepath.rel(
				project_root,
				candidate.candidate_path,
				context.allocator,
			); if error != .None do return {false, "could not make relink path relative"}; asset.project_path = relative
		} else {
			destination := project_asset_record_path(
				project_root,
				asset^,
			); if !project_asset_path_within_root(project_root, destination) do return {false, "copied asset relink destination escapes project root"}
			parent, _ := filepath.split(
				destination,
			); if !os.is_dir(parent) && os.make_directory_all(parent) != nil do return {false, "could not recreate copied asset directory"}
...
project_asset_plan_stage :: proc(
	registry: ^Project_Asset_Registry,
	project_root, package_asset_root: string,
) -> Project_Asset_Stage_Plan {
	result := Project_Asset_Stage_Plan {
		allowed              = true,
		attribution_manifest = "format = \"ProjectAssetAttribution v1\"\n",
	}
	for asset in registry.assets {
		if asset.embed_policy == .External do continue
		if asset.embed_policy == .Prohibited ||
		   !asset.provenance.redistribution_permitted {result.allowed = false; append(&result.diagnostics, authoring_diagnostic_init(.Assets, "project-assets", asset.id, "embed_policy", .Blocking, "asset cannot be redistributed")); continue}
		source := project_asset_record_path(
			project_root,
			asset,
		); if !os.is_file(source) {result.allowed = false; append(&result.diagnostics, authoring_diagnostic_init(.Assets, "project-assets", asset.id, "project_path", .Blocking, "embedded asset file is missing")); continue}
		hash, valid := project_asset_sha256_file(
			source,
		); if !valid.ok || hash != asset.sha256 {result.allowed = false; append(&result.diagnostics, authoring_diagnostic_init(.Assets, "project-assets", asset.id, "sha256", .Blocking, "embedded asset hash does not match the registry")); continue}
		ext := filepath.ext(
			source,
		); package_path, join_error := filepath.join([]string{package_asset_root, fmt.tprintf("%s%s", asset.id, ext)}, context.allocator); if join_error != nil {result.allowed = false; continue}; append(&result.items, Project_Asset_Stage_Item{asset.id, source, package_path, hash, asset.technical.byte_size}); result.total_bytes += asset.technical.byte_size
		result.attribution_manifest = fmt.tprintf(
			"%s\n[[assets]]\nid = \"%s\"\nsha256 = \"%s\"\nsource_uri = \"%s\"\nsource_name = \"%s\"\ncreator = \"%s\"\nattribution = \"%s\"\nlicense_id = \"%s\"\nlicense_text = \"%s\"\n",
			result.attribution_manifest,
			project_asset_io_escape(asset.id),
			asset.sha256,
			project_asset_io_escape(asset.provenance.source_uri),
			project_asset_io_escape(asset.provenance.source_name),
	...
	for usage in registry.external_catalog_usages {
		if !project_external_asset_reference_valid(
			usage.reference,
		) {result.allowed = false; append(&result.diagnostics, authoring_diagnostic_init(.Compatibility, usage.document, usage.entity_id, usage.field_path, .Blocking, "external catalog requirement is not namespaced and version-pinned")); continue}
		duplicate :=
			false; for known in result.external_catalog_requirements do if known == usage.reference {duplicate = true; break}; if !duplicate do append(&result.external_catalog_requirements, usage.reference)
		result.attribution_manifest = fmt.tprintf(
			"%s\n[[external_catalog_requirements]]\nnamespace = \"%s\"\ncatalog_id = \"%s\"\nversion = \"%s\"\ndocument = \"%s\"\nentity_id = \"%s\"\nfield_path = \"%s\"\n",
			result.attribution_manifest,
			project_asset_io_escape(usage.reference.namespace),
			project_asset_io_escape(usage.reference.catalog_id),
			project_asset_io_escape(usage.reference.version),
			project_asset_io_escape(usage.document),
			project_asset_io_escape(usage.entity_id),
			project_asset_io_escape(usage.field_path),
		)
	}
	return result
}

project_asset_stage_plan_destroy :: proc(plan: ^Project_Asset_Stage_Plan) {delete(plan.items)
	delete(plan.diagnostics)
	delete(plan.external_catalog_requirements)
	plan^ = {}}
