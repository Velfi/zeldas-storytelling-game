...
authoring_case_document_path :: proc(
	item: ^Authoring_Case,
	...
	switch kind {
	case .Story:
		return autosave ? item.paths.story_autosave : item.paths.story
	case .Level:
		return autosave ? item.paths.level_autosave : item.paths.level
	case .Graph_Layout:
		return autosave ? item.paths.graph_layout_autosave : item.paths.graph_layout
	}
	return ""
}

authoring_case_mark_dirty :: proc(
	item: ^Authoring_Case,
	kind: Authoring_Document_Kind,
	revision: u64,
) {
...
authoring_case_mark_saved :: proc(
	item: ^Authoring_Case,
	kind: Authoring_Document_Kind,
	revision: u64,
) {
