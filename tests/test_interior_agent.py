...
class InteriorAgentTests(unittest.TestCase):
    ...
    def test_relationship_preview_does_not_mutate(self):
        ...
        AGENT.resolve_placement(self.level, self.catalog, "study", "plant", "against_wall", None, .2, "west", None)
    ...
    def test_on_surface_uses_authored_surface(self):
        ...
        placement = AGENT.resolve_placement(self.level, self.catalog, "master_bedroom", "table_lamp_square", "on_surface", support["id"], .2, "nearest", None)
    ...
    def test_beside_distance_is_clearance_not_center_offset(self):
        target = AGENT.object_for(self.level, "furnish_dining_room_1_table_round")
        placement = AGENT.resolve_placement(self.level, self.catalog, "dining_room", "chair_rounded", "beside", target["id"], .25, "nearest", None)
        assets = AGENT.catalog_objects(self.catalog)
        expected = assets[target["catalog_id"]]["footprint_radius"] + assets["chair_rounded"]["footprint_radius"] + .25
    ...
    def test_engine_commit_preserves_permissions_and_valid_toml(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            target.chmod(0o644)
            placement = AGENT.resolve_placement(self.level, self.catalog, "study", "plant", "center_of_room", None, .2, "nearest", None)
    ...
    def test_engine_preview_is_authoritative_when_built(self):
        ...
        if not engine.exists():
            self.skipTest("engine has not been built")
        placement = AGENT.resolve_placement(self.level, self.catalog, "study", "plant", "against_wall", None, .2, "west", None)
